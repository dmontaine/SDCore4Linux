...
| ✅ | **S.40** | M | ***CLOSED — §WITNESSED§ 22 Sep 2026: `witness-absence.sh --commit` ON THE FRESH INSTALL (`6d18e54`), 111 OF 111 CHECKS PASSED, INCLUDING THE FULL M13a-g2 BATCH-GATE MATRIX*** (owner-run, real sudo session: refusal with no entry, admission once listed, refusal again once delisted, refusal for an argument, refusal for a listed-but-wrong-VOC-type command, `sdsys` elevation bypassing the list, and `batch.jobs`'s own ownership/mode). ***THE BATCH GATE — PARITY WITH THE WINDOWS PORT (owner's ruling, 20 Sep 2026: a behavior prevented on one port must be prevented on both, the mechanism free to differ with the OS).*** Found while explaining Q.22's `batchjob` verifier: a single-command invocation (`sd '<command>'`, `system(1026)`) has run UNCONDITIONALLY on this port since `login`/`cproc` were written — the group-membership test at entry proves WHO is running it, never WHAT it may run unattended.  Windows built `batch.jobs` 22 Aug 2026 (its PROJECT_STATUS.md 7 step 9) for exactly this: `sd.exe` used to refuse every unelevated command line outright, breaking a legitimate scheduled job along with everything else.  ***§BUILT§ 20 Sep 2026, NO BASIC COMPILED AND NOTHING RUN.*** `login`'s `batch.permitted` (ported from the port's own routine, same algorithm, new messages 11000-11002 from this port's own block): admitted by `K$ADMINISTRATOR` or by an exact, argument-free match in the account's own record in `@SDSYS/batch.jobs` (VOC type PA or S only); refused otherwise, before `cproc` ever sees the command.  `batch.jobs` is a directory file (the shape of `accounts`/`$cred`, not a VOC entry — a VOC record grants its own account Modify by default), `sdsys:sdusers 0750` (group may read, never write — matching the port's own "read-only to sdusers" rule), created empty by the installer and preserved by `deletesdai.sh` on a keep cycle exactly as `$cred` is.  Free: `gplbld/test-batchgate-units.py`, 24/24 + `--selftest` 6/6 mutants caught (two genuine holes found and fixed by the self-test itself — see the entry).  Witness M13a-g2 written in `witness-absence.sh`. ***FIRST LIVE CONFIRMATION, BEFORE THE FULL WITNESS RUN, 22 Sep 2026, ON THE FRESH INSTALL (`6d18e54`, deletesdai.sh+installsdai.sh):*** `sd 'PY_LISTCRTE'` as `don` (a plain account, not `sdsys` — the teardown's own administrator, not the pre-teardown ADMINISTRATOR-tier registration) was refused with exactly the M13a wording, unprompted: "PY_LISTCRTE is not a command that account don may run from the command line." `sd 'PY_LISTCRTE zztestlist'` was separately refused for carrying an argument, matching M13c's wording ("must be a single name with nothing"). ***THEN THE FULL MATRIX, SAME INSTALL, OWNER-RUN `witness-absence.sh --commit`: 111 OF 111, M13a THROUGH M13g2 ALL PASS*** — nothing left open on this row | 22 Sep 2026 |
| ✅ | **S.41** | R | ***[S.41] CLOSED, NO GAP — PHYSICAL/CONSOLE LOGIN FOR AN ORDINARY SD ACCOUNT, WINDOWS'S GATE 47 NEXT-PASS QUESTION, 22 SEP 2026*** (`to-linux/2026-09-22T1600-windows-console-login-question.md`, moved `done/`). Windows joins every ordinary account to `sdsshonly`, denying both console and remote-desktop Windows logon rights, so SD's own `os.users` gate is the only way in; asked whether Linux has the same gap open. ***CHECKED, NOT ASSUMED — NO GAP, AND THE ANSWER IS S.27, NOT A NEW BOUNDARY.*** Confirmed no shell override anywhere: `sd-elevate useradd` (`gplbld/sd-elevate:677`) and `createa` pass no `-s`; on this box `don` is `/bin/bash`, `sdsys` is `/bin/sh`, neither `nologin`; no `/etc/securetty`, no `nologin.conf`, `pam_access.so` commented out in `/etc/pam.d/login` — nothing stops a local tty or GDM session, matching what the Windows agent already found by search. ***BUT IT DOESN'T MATTER, BECAUSE OF S.27***: `SH`/`OS.EXECUTE` inside `sd` already run unconditionally at the account's own Linux permissions — there is no SD-level gate left to bypass. An ssh session that runs `SH` and a console/GDM login land at the identical native shell: same UID, same groups, same files reachable. Windows needs `sdsshonly` because `os.users` is a SECOND wall behind which `SH`/`OS.EXECUTE` can be individually denied, and a console session walks around that wall entirely; Linux tore its second wall down on S.27's ruling ("the only limits ... those Linux imposes") and never rebuilt one for console to walk around. ***ALIGNED, MECHANISM DIFFERS BECAUSE THE UNDERLYING BOUNDARY DIFFERS — NOT A GAP.*** Replied `to-windows/` | 22 Sep 2026 |
| ⬜ | **S.42** | S | ***[S.42] PARITY GOES BOTH WAYS (owner, 22 Sep 2026: "if you have a feature that windows does not, then they need to come into parity if possible on their operating system").*** Gate 47's own diff already found two Linux-only items it filed as "divergence, do not fix" under the OLD one-way framing — re-sent to Windows as real asks under the new one. **1)** `nano` VOC alias (`voc_template/nano`, `newvoc/nano`: `V`/`CA`/`$EDIT`, 3-4 lines, `newvoc`'s copy adds `Verb - Full screen editor`) — trivial, same catalogued program their `MICRO`/`EDIT` already point to, no reasoning needed, just add the record. **2)** `UMASK` — Windows called it "essentially inert on Windows where security is ACL-based" (`d913eac`, 24 Aug 2026) and removed it *before* this audit existed; asked them to re-examine under the owner's new bar (build it if *possible*, not only if convenient) rather than let a pre-audit call stand unquestioned — not answered here, since it's their OS's feasibility call. Sent `to-windows/2026-09-22T1645-linux-parity-both-ways.md` | — |
| ✅ | **S.42a** | — | ***[S.42a] CLOSED — WINDOWS'S REPLY, SAME DAY*** (`to-linux/2026-09-22T1700-windows-parity-both-ways-reply.md`, `done/`). **1) `nano` — built** on their side, byte-identical shape to their own `micro`/`edit` records; not yet cycled/witnessed there, rides their next milestone cycle. **2) `UMASK` — re-examined, not reasserted; answer stands, for a checked reason now.** Windows already has the same job done, inverted and stronger: the installer sets one inheritable ACE on `C:\ProgramData\SD` once, and NTFS applies it at file-creation time to everything created underneath *below the runtime* (verified 13 Aug 2026, their §5.7) — where Linux's `umask` is per-session, mutable, easy to forget, Windows's mechanism is one-time, structural, can't be forgotten. No verb has anything left to control: there's no per-process mode-bits value their SD passes when it writes a file for a `UMASK`-shaped verb to expose. **Accepted — the job is already done, more strongly, a different way; nothing to build.** **3)** their own verb-surface sweep found a genuine gap in *this* tree by content-diffing every shared `gpl.bp` file, not presence alone — see S.43 | 22 Sep 2026 |
| ✅ | **S.43** | S | ***CLOSED — §WITNESSED§ 22 Sep 2026: A REAL CALLING BASIC PROGRAM, NOT JUST A COMPILE CHECK.*** `zztest.pylist`, run in `don`'s own account (throwaway, cleaned out of `bp`/`bp.out` after, no VOC record made): `PY_LISTCRTE('zzwitnesslist')` → status 0; appended a string object to it (`PY_STRSET`+`PY_LISTAPPD`, both status 0); `PY_LISTGETS` read back `hello`; `PY_OBJLEN` → 1; a second `PY_LISTCRTE` on the same name correctly refused with status -12038 (`LstExsts`), matching the documented code exactly. ***A REAL DOCS BUG FOUND BY RUNNING IT, NOT BY READING SOURCE***: `PY_OBJTYPE` returned `2`, not the string "list" as first documented — checked `sdext_py.c`'s `PyObjType()`, it's a six-way numeric code (`SD_Obj_Unkn`/`Str`/`List`/`Dict`/`Long`/`Float`, `gplsrc/keys.h`), fixed in the chapter (`SDCoreLinuxDocs` `f67a46f`) before it shipped wrong. ***[S.43] `PY_LISTCRTE` — THE EMBEDDED-PYTHON API COULD NOT CREATE A LIST, FOUND BY THE WINDOWS PORT'S OWN VERB-SURFACE SWEEP, 22 SEP 2026*** (`to-linux/2026-09-22T1700-windows-parity-both-ways-reply.md`, item 3). `SD_PyListCrte` (2220) was reserved in `gplsrc/keys.h` and dispatched by `op_sdpyobj.c`, but nothing implemented or called it — `PyListAppd`/`PyListGet`/`PyListClr` all existed, `PyListCrte` did not, so a list could be appended to and read but never made. Windows hit and fixed the identical gap 14 Sep 2026 (`py_listcreate`); this tree had never independently found it. ***§BUILT§ 22 Sep 2026, C-COMPILED CLEAN (0 errors, 0 warnings), NOT YET CYCLED OR WITNESSED*** — matches how Windows itself reported its own `nano` fix today, honestly, not claimed further than checked. `PyListCrte` (`gplsrc/sdext_py.c`, mirrors `PyDictCrte`'s shape exactly: `PyList_New()`/`PyMapping_SetItemString` against `global_dict`, not Windows' `py_call("LISTCRTE",..)` helper-process dispatch — same result, this tree's own architecture) + dispatch case (`op_sdpyobj.c`) + BASIC wrapper `sdsys/gpl.bp/py_listcrte` (named for this tree's own `py_list<verb>` convention — `listappd`/`listclr`/`listgets` — not Windows' `py_listcreate`) + uncommented the reserved opcode in both `gplsrc/keys.h` and the BASIC-visible `sdsys/syscom/keys.h` (a second, separately-maintained copy — the C header alone would not have been enough) + two new error codes `SD_PyErr_LstCrteEr`/`SD_PyErr_LstExsts` (`gplsrc/err.h` -12037/-12038, this tree's own numbering, already diverged from Windows's here — precedent already set by `LstAppdEr`/`LstClrEr`, no mailbox check needed for this range). Regenerated derived headers (`gplbld/gen_includes.py`) before `make` would pass `check-includes`. ***CHANGELOG GAP FOUND AND FIXED*** (while checking release readiness for S.23): the commit that built `PyListCrte` never touched `sdsys/changelog`, so a user-visible new verb had no entry — added (22 Sep 26, ahead of "20 Sep 26  A COMMAND RUN..."). ***THE INSTALL/CATALOG CYCLE RAN, 22 Sep 2026, ON `6d18e54`***: `/usr/local/sdsys/gpl.bp.out/py_listcrte` and `gcat/!PY_LISTCRTE` both exist post-install — the two-stage bootstrap landed it. ***A SECOND, BIGGER DOCUMENTATION GAP FOUND WHILE CHECKING***: `SDCoreLinuxDocs` had no embedded-Python reference at all — `User/36`'s pointer to "the Administrator set's Encryption and the SDEXT interface chapter" actually lands on the unrelated crypto primitives, not `SDPYOBJ()`. Coordinated with the Windows agent by mailbox first (`to-linux/2026-09-22T1853/1857/1905`, replied `to-windows/2026-09-23T0157`, all moved `done/`) rather than both sides writing it independently: confirmed no permission gate exists on this port's Python calls (same shape as S.27's `SH`/`OS.EXECUTE`, unlike Windows's `os.users`-gated helper process), confirmed error codes -12001..-12034 are identical name-for-name across both ports and -12035 up is genuinely per-port (Windows reuses `NotList`/`ObjNOF`/`LstItem`/`Excpt`/`DictExsts` instead of minting its own), and confirmed all 21 function names match except `PY_LISTCRTE` vs Windows's `PY_LISTCREATE`. Chapter written (`Administrator/12-sd-embedded-python.md`, `SDCoreLinuxDocs` `1fbfbdf`, `PY_OBJTYPE` fix `f67a46f`), rendered, `checklinks.py` 0 broken. Nothing left open on this row | 22 Sep 2026 |
| ✅ | **S.44** | M | ***[S.44] CLOSED 22 SEP 2026 — ARCH, FEDORA AND OPENSUSE RESTORED*** (owner, same day: "update the installer to allow installation to all four supported distributions"). `installsdai.sh` refused every non-Debian family since 09 Sep ("deliberately and temporarily"); pacman/dnf/zypper branches restored from the pre-09-Sep tree, carrying the two real dependency deltas the Debian branch picked up since (`libbsd`/`-devel` dropped S.18, an OpenSSL dev package added S.19) so all four ship the same actual requirements, not a stale copy. **A real bug found doing it, not assumed**: `detect_distro()`'s colon-joined pattern needed a colon BEFORE a family name to match, which Linux Mint and Pop!_OS's `ID_LIKE`-only os-release never has (Mint: `ID_LIKE=ubuntu`, nothing before it) — rewritten to space-bounded word matching. **A second real gap, confirmed against this tree's own PROJECT_STATUS (row A2)**: nano's `/usr/share/nano/*.nanorc` glob is Debian's own patch to `/etc/nanorc`, not upstream nano — placing the file on Fedora/Arch/openSUSE would have shipped a file nano never reads. The installer now runs the same reachability check `verify-editors.py`'s `active_includes()`/`include_covers()` already use and appends one exact include line where it's missing; `deletesdai.sh` removes it by the same exact match. **Also, while auditing for anything Debian-specific (owner's item 4)**: the closing summary claimed "a ufw allow rule was added" unconditionally, which is false on the three distros that ship no ufw (Fedora/openSUSE ship firewalld, Arch ships nothing) even though the `ufw` calls themselves were already safely guarded — now conditioned on one `have_ufw` flag computed once. And `gpl.bp/EDIT`'s missing-editor message named `sudo apt install` unconditionally — a real product-facing bug, would have told a Fedora user a command that does not exist on their machine; new `find.pkg.mgr` probes apt-get/dnf/pacman/zypper in turn (compile-checked clean, `0 error(s)`, via a dev build — see S.45). **Owner's item 2, the "tested on" line**: reinstated (`Installer tested on Ubuntu 26.10.`), reversing the 20 Sep removal on the owner's direct instruction of 22 Sep; the 20 Sep reasoning is not wrong, the owner chose to pay that cost again. **A third real gap, this one in the build itself, not the installer**: `Makefile`'s `LIBSODIUM := /usr/lib/x86_64-linux-gnu/` is Debian/Ubuntu's own multiarch triplet and does not exist on Fedora, Arch or openSUSE — `make` would have failed to link on any of the three even with the package correctly installed by S.44's own new branches. Confirmed unnecessary even on Debian: `-lm`/`-ldl`/`OPENSSL_LIBS` already link with no explicit `-L` at all, `gcc -print-search-dirs` already lists that exact directory as one of gcc's own defaults for this target, and a rebuild with the `-L` dropped still links libsodium correctly (`ldd`) on this machine. `bash -n` clean on both scripts, no BOM, 0 CR; `make` rebuilds clean, `sd --version` unaffected. **Left, honestly: not actually installed on Arch, Fedora or openSUSE** — the four package lists are ported from a working prior version plus two dependency deltas already proven correct on Debian, not freshly verified against a live install of any of the other three; that verification is an owner-run cycle, not something this session could produce | 22 Sep 2026 |
| ✅ | **S.45** | M | ***[S.45] CLOSED 22 SEP 2026 — SHIP TIME: L1.1-0*** (owner, same day: "make sure ... this release is SD Core L1.1-0"). `sdsys/changelog` had read "L1.1-0 - in progress" since 14 Sep while `SD_REV_STAMP` deliberately stayed `L1.0-0` — the owner's own 14 Sep ruling was to bump only at ship time, exactly as the Windows port keeps `W1.0-0` while its changelog reads "W1.1-0 - in progress"; today's instruction IS that ship-time trigger. Bumped `gplsrc/revstamp.h` (the source), regenerated the BASIC copy via `gplbld/gen_includes.py` (`make check-includes` confirms all four generated headers in sync), and both `$release` VOC records (`voc_template`, `newvoc`) per the header's own instruction. `MAJOR_REV`/`MINOR_REV`/`BUILD` untouched — they stay at upstream's 1/0/2, a binary interface not a name, per the 14 Sep ruling. ***WITNESSED ON A DEV BUILD, NOT JUST COMPILED***: `rm gplobj/*.o && make EXTRA_C_FLAGS=-DSD_DEV_BUILD` then `./bin/sd --version` → `String Database (sd) Version L1.1-0 64 Bit`; same build used to compile-check S.44's `gpl.bp/EDIT` change (`0 error(s)`, staged in and cleaned out of `don`'s own `bp`/`bp.out`, no VOC record left behind); rebuilt with plain `make` after, confirmed still `L1.1-0` and the DEVELOPER BUILD banner gone. **Owner's item 3's other half — "for Windows" anywhere it is DISPLAYED, not anywhere it is mentioned**: the actual sign-on banner already omits platform branding entirely by a 9 Sep ruling ("the platform is carried by the L in the stamp") — checked and correctly left alone. The dozens of `for Windows` hits elsewhere are source comments citing the port for parity/provenance and are accurate; **three real ones found in the shipped message catalog** (`sdsys/messages/2005/3123/6076` — "not available on Windows", "Preserve case in Windows directory file select", "Passwords are set using Windows administration tools"), confirmed unreachable from any call site in this entire tree (upstream cruft, inherited, never wired), reworded to drop the false platform claim rather than deleted (numbers stay allocated, `test-msgreserved-units.py` still 1923/1923 in range). Docs: five `L1.0-0` mentions in `SDCoreLinuxDocs` (a live `:config`/`system(1012)` transcript, the `$release` VOC example, two on the Getting-Started "Start here" page) bumped to `L1.1-0` to match. **Left, honestly: `sdsys/changelog`'s own header still reads "in progress"** — deliberately not touched, since declaring the changelog itself released is a separate, bigger action (tagging, staging repositories, a release zip — S.23, not started) that was not asked for; only the revstamp and the docs that quote it were in scope here | 22 Sep 2026 |
...
| ◐ | **Q.22** | L | verifier harness and eleven verifiers; `keys` 36/36 and `logtoaccess` (§2b) witnessed on `984be50`; `batchjob`/`cmdaudit` mechanism absent, `notyet` → Q.14; `sdsyswrite` built 14 Sep as `check-storewriters.py` (free, 4 writers 0 failures) + witness §13g; §13g Y1-Y4 pass on `dea3736` and `0d58171` but Y0 shows the session never took the LOGTO route; route fixed 15 Sep (`LOGTO zzrel1` before the first WHO) and `sdsyswrite` witnessed on `0b67dba`, Y0–Y4 all pass (WHO `zzrel1 sdsys`); ***the ten API verifiers RULED 15 Sep 2026*** once W.4 closed (PORT_ADOPTION "Queue 22 — the ten API verifiers, ruled"): seven already measured by witness rows, `localconnect` does not transfer (phase 5 removed that transport), `apiname` was a real hole — `!valid_os_name` screens the SCRAM name at `apisrvr:1100` and nothing had ever sent it one it refuses — now built as §13c S9/S9b and ***WITNESSED 15 Sep 2026***: 271/271 on `c773008` (16:18) and again on `c1ea29b` (18:26), S9 refused at 47 and S9b reading `reason=name rejected by valid_os_name` off the trail — the row that proves the name check fired and not a catch-all; ~~`tierapi`'s STANDARD leg is built 15 Sep 2026 and has not run: §13j J0–J4c...~~ ***§13j IS STRUCK, NOT LEFT — THIS ROW WENT STALE THE DAY THE TEARDOWN LANDED AND WAS NEVER CORRECTED.*** S.25 (18 Sep) removed the tier model entirely; `witness-release-run.sh` says so at its own §13j header (*"Q.22 tierapi - struck: there is one layer, and `witness-absence.sh` proves it"*) and has since 18 Sep — there is no STANDARD or PROGRAMMER account left for J0–J4c to test, and the upward-refusal it measured was tier gate 13b's A4, already struck the same day. J5's own-voc write question stays answered by source, not by a witness: an API session runs as the account's own user, and its own VOC records are its own files (`createa:391-393`/`:724-729`).  ***THE ONLY GENUINE REMAINDER***: `batchjob`/`cmdaudit` have no mechanism to verify because the mechanism they would verify does not exist — no `batch.jobs` in `gpl.bp`/`sdsys`, no `batch.command` in `login` — so either is an ADOPTION (build the feature) before it could be a verifier, not a quick check; lower value, per PORT_ADOPTION's own ranking.  Left: `batchjob`/`cmdaudit`, and only those | — |
...
| ✅ | **P.31** | S | ***CLOSED — §WITNESSED§ 20 Sep 2026, FOUND WHILE ANSWERING A DIFFERENT QUESTION.*** `witness-release-run --commit`, `/var/tmp/witness-release-run.20260920-155539.log`: K8 *"10188 named zzrel2, whose voc was missing: found 'Account zzrel2 could not be opened'"*, K9 *"and DELETE.ACCOUNT still finished (register record gone)"* — both PASS.  `delacc:219`'s cross-reference scan opened every other account's `voc` with no ELSE, so one unopenable `voc` aborted DELETE.ACCOUNT before the confirmation instead of skipping that account — the Windows port's find (its RELEASE_1.1 44), confirmed here by reading; shared BASIC, so upstream too.  ***FIXED 15 Sep 2026 with the port's number and wording, 10188*** (it built first, so it carries the number).  §15 moves `zzrel2`'s `voc` aside for the one `DELETE.ACCOUNT` and puts it straight back afterwards; the row makes the genuinely-missing-`voc` case happen rather than hoping a real one turns up | 20 Sep 2026 |
| ✅ | **S.24** | S | ***CLOSED — §WITNESSED§ 20 Sep 2026, THROUGH ORDINARY USE RATHER THAN A DEDICATED ROW.*** The fix (`475cb71`/`8a45a12`, 15 Sep) is a confirmed ancestor of today's install (`b8e195d`, `git merge-base --is-ancestor` checked), so every session run today built with it in place; none set `SD_CONFIG` or `SCARLET_CONFIG`, and `witness-absence` (99/99, 96/96) and `witness-release-run` (248/249, 247/249) ran hundreds of `sd` sessions on it with no config-path failure — the exact "sets nothing" case the entry needed measured.  The configuration file was named by two environment variables — the server read `SCARLET_CONFIG` (`inipath.c:38`), the client library `SD_CONFIG` (`sdclilib.c:4080`) — so setting either moved half the system and said nothing; the port settled this on 14 Aug 2026 and its decision is this port's.  Fixed: `SD_CONFIG_ENV`/`SD_CONFIG_DEFAULT` in `sddefs.h`, the symbol read in `inipath.c` with a bounded copy, `sdfix.c`'s `read_sdconfig()` buffer widened from `200 + 1` to `MAX_PATHNAME_LEN + 1`; guarded by `test-configpath-units.py` 14/14 + `--selftest` 8 mutants | 20 Sep 2026 |
| ✅ | **W.4** | XL | API surface walked 14 Sep; the tier gate and lower-case WHO witnessed on `3ff8027` (§13b A4.3, A1b); SCRAM phase 1 (primitives) built, RFC 7677 17/17; phase 2 (`$cred`, MODIFY.PASSWORD) witnessed on `74c60d4` (§13 C0–C7), its two fixes on `b119bb3` (§15 K7, `verify-setpw` 22/22); phase 3 (APISRVR 47/48, K$SET.USERNAME/K$ASSUME.USER) witnessed on `d880012` (§13c S1–S7); phase 4 (client `scram_login`, both `sdclilib.c` copies) witnessed on `85fbbec` (§13b A0–A5, §13c S5c); phase 5 (request 24 retired, `!sdclient` SCRAM, `SDConnectUDS` removed) witnessed on `9fd52d9` (§13c S5–S5e, §13d B0–B4b, 171/171); phase 6: both client libraries rebuilt (measured); SD passwords — ***REVERSED 18 Sep 2026 and RESTORED 19 Sep as REQUIRED (owner, via the port's RELEASE_1.1 70): asked up to three times, verified in `$cred`, via the root loginuid bridge; see START HERE "19 SEP 2026, EARLY"*** — `installsdai.sh` then ended by setting the installing user's with `sudo sd -QUIET MODIFY.PASSWORD` (owner, 15 Sep 2026: that user only, not SDSYS, whose SD password nothing on Linux uses — the port sets both, its PRE_RELEASE_FIXES 138), skipped when a keep cycle kept one, ***WITNESSED 15 Sep 2026 ON INSTALL `c773008` (15:44:32): the owner was asked at the keyboard (his word), and `don` went from refused at request 47 — no credential, the 10:09 install — to refused at 48 with a real salt and `i=600000`, measured by `scram-probe` with a deliberately wrong password.*** No instrument can run the prompt itself (`input … hidden` needs a tty); any other account stays a per-account `sudo sd` + MODIFY.PASSWORD job. Witness 269/269 on the same install | 15 Sep 2026 |
...
| ✅ | **S.22** | L | ***PRE-RELEASE 2 of 3 (owner, 15 Sep 2026): the documentation — CLOSED 22 SEP 2026.*** The Linux side starts from the UPDATED WINDOWS documentation and changes it where Linux differs. In `SDCoreLinuxDocs` (a separate repository, `github.com/dmontaine/SDCoreLinuxDocs`, `ad1ef1c`), not this one. **`GettingStarted` (19/19), `Administrator` (14/14) and `User` (52/52) each had a dedicated read this session**, not a keyword sweep. `tools/docmap.py`/`tclmap.py`/`confmap.py` clean throughout, re-run clean after the final commit. Real corrections found, not just reworded: `update.accounts all` is not run automatically on upgrade here (a genuine gap vs Windows, not a difference); `sdtic` does not survive on the installed machine; `edit` aliases `ed`, not a full-screen editor — `nano`/`micro` are the two real ones; `writeseq`/CSV write LF here, not Windows's deliberate CRLF; `umask` is a real dispatchable verb, not unreachable as the `User` set claimed; a login-paragraph example still read `TERM WINDOWS`, fixed to the real `TERM ANSI`; `config gpl`/`config contrib` were documented as reading a VOC X-record — they shell out to `less` against plain files instead; VOC record-type and verb-dispatch counts recounted directly from `voc_template`/`newvoc` against fabricated-looking figures the pages carried. Fixed 11 cross-set hyperlinks this session's own edits had introduced. Left, honestly: the PDF-rendering toolchain (`mkpdf.ps1` has no Linux port); whether account directories are permissioned to isolate accounts from each other is stated as unverified in Security rather than assumed; some disclaimer swaps done by analogy not fresh re-verification, one page (18) deliberately left unswapped | 22 Sep 2026 |
| ◐ | **S.23** | M | ***PRE-RELEASE 3 of 3 (owner, 15 Sep 2026): staging repositories per version, then a release zip of each — one Linux, one Windows.*** ***THE SHAPE IS RULED 15 Sep 2026 — "the same as the windows version, staging directory and then a zip"***, so the zip is a distribution artifact assembled outside the project (the port's model, read out of its record) and the "installer builds from an unpacked zip" option is dropped. Costed 15 Sep against the code: pinning a TAG in the shipped `installsdai.sh` is one value (`:75`; `--branch` already takes tags), the stamp stays exact (`:397`, `:688-690`) and `assert-current` is unaffected (reads only `commit`, strict identity, fails pessimistic) — the cost is that a development install must keep cloning `main`, so the shipped and repository scripts would differ. ***OWNER'S YES ON THE TAG, 22 Sep 2026: PIN IT.*** `v1.1-0` tagged and pushed on both `sdcore4linux` (`f0cf824`) and `SDCoreLinuxDocs` (`f67a46f`) — neither had ever been tagged before (checked both remotes directly; Windows's own staging draft claims a `v1.1-0` tag that does not actually exist on its remotes either, at time of writing). ***BUILT, 22 Sep 2026***: `/home/don/Projects/SDCoreLinuxProject/SDCore-L1.1-0/` — `installsdai.sh` (repo copy `REPO_BRANCH` changed from `main` to `v1.1-0`, this one file only, not the tracked source, matching the costed divergence above; `bash -n` clean, no BOM), `documentation/` (the three book PDFs, rebuilt from today's doc changes, not the stale ones the prior session left — `Administrator.pdf` 64→70 pages, `User.pdf` unchanged at 333, both spot-checked for a real bookmark title, `checklinks.py` 0 broken first), `clients/Linux64/` (`sdclilib.so`/`libsdcli.so`, confirmed distinct content, `make` confirms current), `README.txt` and `SDCore-L1.1-0-what-is-in-the-zip.txt` (Linux-true content, not a translation of Windows's draft — no tier keywords, no `certutil`, sdsys reached by local Linux login not "signing in as SDSYS elevated", no "if you used L1.0-0" section since none shipped). Zipped: `SDCore-L1.1-0.zip`, 11 files. ***THE PINNED CLONE WAS ACTUALLY TESTED***, not assumed: `git clone --branch v1.1-0` against the real `origin` in a scratch dir resolved to `f0cf824`, matching the tag exactly. Availability notice written to `/home/don/Documents/SD-Core-L1.1-0-Availability-Notice.txt`, grounded in `sdsys/changelog`'s own entries (batch gate, TLS 1.3, the one-administrator model, password policy, the lower-case migration — real on Linux because the filesystem is case-sensitive, unlike Windows's "both spellings work anyway"), not a one-for-one translation of Windows's Python/helper-process claims, which don't hold here (Linux embeds Python in-process, no helper). ***OWNER'S CORRECTION, 22 Sep 2026: EVERY "WHERE THE SOURCE IS" MENTION ALSO NAMES THE SOURCEFORGE DOWNLOAD*** (`https://sourceforge.net/projects/sdcore/files/`, matching Windows's own pattern) — added to `README.txt`, `SDCore-L1.1-0-what-is-in-the-zip.txt` and the availability notice, all three places that named GitHub alone; re-zipped, SHA256 then `d0d21dbe382109349845451579281df82a970bf0760544180a466bc43701976c`. ***THE TAG MOVED, 26 Sep 2026*** — the LGPL/GPL license fixes (`820c4a3`, `95cc9b4`, see `sdclilib.h`/`scram_client.h`/`winsdclilib.c`/`sdsys/licence`/`sdb_ai/LICENSE`) needed to reach the shipped installer, so `v1.1-0` was force-moved from `f0cf824` to `95cc9b4` (owner's yes, same day) and force-pushed to `origin`. ***RE-TESTED, NOT ASSUMED***: `git clone --branch v1.1-0` against the real `origin` in a scratch dir now resolves to `95cc9b4`, and the clone carries the fix (grepped, no stray `LGPL` outside the verbatim GPLv3 "why-not-lgpl" epilogue line, which is FSF boilerplate and untouched). `clients/Linux64/sdclilib.so`/`libsdcli.so` rebuilt from current `main` (`make`, still two distinct files by `md5sum`) and restaged; `installsdai.sh` unchanged since 22 Sep (`bash -n` clean, no BOM, checksum in `README.txt` still matches: `a5f525c6…bb4c65`) so its `REPO_BRANCH="v1.1-0"` pin needed no edit, only the tag it names moved. Re-zipped: SHA256 now `7600a58b0020935a4cacc619818269ebe2abfb643a2d6e453f3d5c5e4314d36d`. §OPEN§ — left: the owner's own read of the notice and README content before anything is distributed; no upload to SourceForge or anywhere else was attempted or is in scope here | 26 Sep 2026 |
...
`~/Projects/SDCoreLinuxProject/sdcore4linux` (its documented home since 16 Sep).
The runtime's workspace folder `~/Projects/sdcore4linux` is an EMPTY STUB - do
not work there.  The parity plan is `/home/don/Documents/claude_plan.md`.
...
- ***THE NEXT STEP IS ONE CYCLE, AND IT WITNESSES ALL FOUR AT ONCE.***  Not as
  root (both scripts refuse it and call `sudo` themselves):
  `bash /home/don/Projects/SDCoreLinuxProject/sdcore4linux/deletesdai.sh`
  then `bash /home/don/Projects/SDCoreLinuxProject/sdcore4linux/installsdai.sh`.
  ***A PERSON MUST BE AT THE KEYBOARD***: the install now asks for SDSYS's SD
  password, which is NEW and separate from its Linux one, and Enter skips it —
  ***skip it and S.33's E3c/E3d cannot run***, because they need a credential
  to present.  Then, each WITH `sudo`:
  `sudo bash …/sdb_ai/sd64/gplbld/witness-absence.sh --commit`,
  `…/witness-release-run.sh --commit`, `…/witness-accounts.sh --commit`.
- **The rows written blind this session, in the order they matter.**  §13e
  **E3c** is the control — sdsys over the Unix socket AS sdsys must still be
  ADMITTED, and without it that section is nothing but refusals; **E3d** is
  what S.33 exists for — sdsys's CORRECT password, socket opened by `zzrel1`,
  must be REFUSED and audited by opener.  `witness-absence` **M12a-d** (M12b
  distinguishes 10191 from 10195, or a right refusal for the wrong reason
  reads as a pass), **M2h** and **M11a2** (10921 at both password prompts).
  ***EXPECT AT LEAST ONE INSTRUMENT FIX*** — every cycle so far has needed
  one, and `sprobe_as` is new code that has never run.
- **Mailbox**: `to-linux/` empty, everything handled and filed to `done/`.  The
  port is owed ONE thing and it is in `to-windows/` already: the commit id of
  the renumber, so it can turn its COLLISION declaration into a RESERVED one.
- ***THE OWNER IS THE OWNER OF BOTH PORTS*** (he said so, 20 Sep).  Both agents
  had spent the night writing "your owner" and "our owner" and putting the same
  questions to him twice.  A parity decision needs him once, not once per port.
- **Still open and not started**: the ~419 upstream orphan messages (a
  per-subsystem judgement — see S.34, and ***an orphan scan must ask a QUESTION,
  never produce a deletion list***); an orphan/cross-tree scan as a standing
  REPORT here to match the port's; W.11, the security-model evaluation, which
  now has two worked examples of what it should look for — a capability safe
  only because nobody turned it on.
- **Traps paid for tonight, all recorded in their entries**: a header change
  with only the edited objects cleared (the link fails on an undefined
  reference that reads like a header error — clear every object); `struct
  ucred` needs `_GNU_SOURCE`, which `-std=gnu17` does not imply; and the
  orphan that was unfinished rather than dead.

***HANDOFF, 19 SEP 2026, EVENING — READ THIS FIRST.***
...
- ***[S.36] THE TEARDOWN'S SIX MESSAGES MOVE TO 10190-10195 — DONE 20 Sep
  2026.***  SD Core for Windows had independently allocated 10176-10181 for
  its case-insensitivity conversion (`configf`, `createf`, `upgrade_nocase`,
  four verifiers), so every one of the six meant two different things across
  the ports.  ***NEITHER BLOCK HAD SENIORITY***: upstream `sdb64` at `8a7f63e`
  has 30 records in the 10000 range and the highest is 10029, so both were
  allocated after the ports diverged and provenance could not settle it.
  - **Settled between the two agents**, on the owner's delegation of 20 Sep
    2026 (*"go ahead and decide between the two how to synchronize the two
    systems"*), after he also told us what neither agent had realised — ***HE
    IS THE OWNER OF BOTH PORTS***, so the night's talk of "your owner" and
    "our owner" was a fiction and the same questions had been put to him
    twice from two machines.
  - **This port moved, and the reason is the cost of PROOF rather than the
    cost of editing.**  Windows counted 27 real sites against our 44 — closer
    than either agent guessed — but a cycle is owed here for S.32/S.33/S.35,
    so the new ids get witnessed in a run that has to happen anyway, while
    the Windows verifiers cannot run at all until its own LOGTO SDSYS problem
    is fixed.  27 sites that cannot be re-proved beat 44 that can be
    re-proved for nothing.
  - **Mapping**: 10176→10190, 10177→10191, 10178→10192, 10179→10193,
    10180→10194, 10181→10195.  ***EVERY RECORD COPIED BYTE-FOR-BYTE AND
    VERIFIED WITH `cmp` BEFORE THE OLD ONE WAS DELETED*** — this is a
    renumber and not a rewording, and nothing a user sees changed.
  - ***PROJECT_STATUS HISTORY IS NOT REWRITTEN.***  61 of the 105 references
    were in dated entries describing what happened on 18 and 19 Sep; those
    keep the numbers they used at the time, because rewriting an entry to
    match today is the opposite of what this document is for.  Only
    current-state claims — the task table and S.35's entry — were corrected,
    and `cproc` and `modifya` carry a dated history line recording the move.
  - **The port declares it the moment this is pushed**: its
    `test-msgreserved-units.py` holds the six as a COLLISION today and will
    hold 10190-10195 as RESERVED (must stay ABSENT there) instead.
  - Also settled under the same delegation, without the owner: the cross-tree
    scan stays a REPORT on both sides and gates nothing, printing the commit
    and age of each tree it read; and there is ***no shared register*** — the
    scan plus the mailbox caught everything a register would have, and a
    register nobody maintains is worse than a scan somebody runs.  The
    trailing space on 6131/6135/6140 is the one thing the Windows agent would
    NOT take on delegation, because it edits text a user sees in its own
    product on the strength of our report; it is asking the owner directly.
- ***[S.35] SDSYS HAS NO REMOTE ACCESS AT ALL — CLOSED, §WITNESSED§
  20 Sep 2026 on the S.38 cycle's `witness-absence --commit`: M12a-d, 4 of
  4.***  Owner, 19 Sep 2026: *"sdsys should not have any remote access from
  ssh or api"*.  ***Its message is 10191 since S.36; the entry
  below says 10177 because that is what it was written as.***
  - ***THE EARLIER REPORT OF A SILENT FALL-THROUGH WAS WRONG, AND THE
    CORRECTION MATTERS MORE THAN THE CHANGE.***  This session told the owner
    a remote sdsys session was "neither granted nor refused — it just becomes
    an ordinary session".  Measured after his ruling: `login:334` refuses it
    entry to the SDSYS account with 10002, audited.  ***THE DOOR WAS SHUT.***
    What was true is that it was shut one room further in, at account entry
    rather than at the session door.
  - **What changed**: `cproc`'s entry block gains the `else` arm the local
    test never had.  A sdsys session with `SSH_CONNECTION` or `SSH_TTY` set is
    refused before it starts, in 10177's words, audited `reason=sdsys session
    from a remote transport` — the treatment root (10176) and a sdsys session
    without a sdsys login (10181) already get.  **A session that should not
    exist should not start.**
  - **10177 came back for this** — see S.34.  Deleted as an orphan earlier the
    same evening, restored with its original bytes and wired here, because its
    text was written for exactly this case and only the wiring was missing.
  - **The API half needed nothing new**: S.33 already refuses a remote sdsys
    API session (10174) and one whose socket peer is not sdsys (10922).  The
    API does not pass through CPROC, which the new comment says so the next
    reader does not look for it there.
  - **What would falsify it**: M12a-d on the next cycle.  M12b is the row that
    stops a false pass — the bridge keeps the session otherwise GOOD (OS user
    sdsys, loginuid sdsys), so if 10181 fired instead of 10177 the refusal
    would be right for the wrong reason and M12a alone could not tell.
  - ***THE LIMIT, STATED: THIS MEASURES SD'S HALF ONLY.***  A real ssh login as
    sdsys cannot be staged — sshd's `DenyUsers` refuses it, which is the other
    half and is checked against the config by `test-ssh-forcecommand.py`.  M12
    sets `SSH_CONNECTION` to reproduce what CPROC actually reads.  It proves SD
    refuses, not that sshd does.
- ***[S.34] DONE 19 Sep 2026 — FOUR MESSAGE RECORDS WITH NO CALLER; THREE
  DELETED, AND 10177 RESTORED AND WIRED THE SAME NIGHT (S.35) WHEN THE OWNER
  RULED THE CASE IT DESCRIBED MUST BE REFUSED.***  ***THE CORRECTION IS THE
  FINDING: AN ORPHANED MESSAGE MEANS "UNFINISHED" AS OFTEN AS IT MEANS "DEAD",
  AND NOTHING IN THE TREE CAN TELL YOU WHICH — ONLY A RULING CAN.***  The
  port's own framing said exactly that ("either dead text or a deletion that
  did not finish") and this session read 10177 as the first when it was the
  second.  A standing orphan scan must therefore produce a QUESTION, never a
  deletion list.
  ...
  - **10177** — written in the teardown (`e41d318`) and ***never wired***:
    ...
    ***DELETED HERE AND THEN RESTORED AND WIRED THE SAME NIGHT (S.35)***, when
    the owner ruled that sdsys must have no remote access at all: the text was
    right and only the wiring was missing, so it came back with its original
    bytes rather than as a new number.
...
- ***[S.32] THE ATTEMPT COUNT IS MESSAGE 10921 — CLOSED, §WITNESSED§
  20 Sep 2026 on the S.38 cycle's `witness-absence --commit`.***  Both
  programs, filled in, neither falsifier found: `set_passwd` (M2h, "attempt
  1 of 3") and `set_acc_password` (M11a2, "attempt 3 of 3").  The port took
  10921 ("That was attempt %1 of %2.") on 19 Sep 13:40 and we agreed it at
  14:00, so ours moved off a hard-coded `crt` and onto the message:
  `set_acc_password:294` (a weak entry) and `:302` (a mismatch),
  `messages/10921` new.  ***`set_passwd` gained a count it never printed***
  — its header claimed MODIFY.PASSWORD's shape and it had three attempts
  with no way to tell which one you were on.  *What would have falsified
  this*: either prompt printing a bare number, an empty expansion, or 10921
  not found — none did.  The installer's own sdsys prompt still says
  "(attempt N of 3)" in its own words — it runs before SD is reachable and
  cannot call `sysmsg`; left alone deliberately, not missed.
...
owner's four MODIFY.PASSWORD checks at the keyboard.  S.25-S.28 (the
teardown), W.10 and S.31 are closed.  NEXT: the security-model evaluation
the owner reserved for after the teardown was witnessed (W.11, his); 10921
for our attempt lines (S.31's row); S.29; a keep-configuration cycle for
R3.  Nothing is unmeasured on the install.***

*Earlier the same day:*  **State (19 Sep 2026, ~10:30).**  ***THE INSTALL IS `359d5ce` (the fifth
cycle's) AND ITS WITNESSES RAN: absence 59/60, release-run 218/224, accounts
32/32; `$cred/don` PRESENT before the delete and after the install (the
required install password's evidence, both times); the owner's keyboard
check of W.10 passed except one fault.  HEAD carries the fixes, UNMEASURED;
THE NEXT STEP IS THE SIXTH CYCLE (same recipe).***

***HANDOFF, 19 SEP 2026, ~10:30 — READ THIS FIRST.***  What the fifth cycle
found, and what HEAD changes (none of it installed):
1. Product: DELETE.ACCOUNT left `/home/sd/user_accounts/zzrel1` (a 755
   `zzdeep` subtree the user owned; sdsys cannot remove it) and said nothing
   - `delacc` discarded OS$DELETE's result.  Now `sd-elevate rmtree-account`
   (exactly a direct child of `user_accounts`/`group_accounts`, canonical
   path) when not root, and 10919 names a directory still there.  10919 is
   NEW in the shared number space (free in both ports; mailed).  10158 lost
   "can be given a tier again".  Witness: release-run K rows + cleanup line
   must read `dir=no`.
2. Product: `MODIFY.PASSWORD` (own, ordinary user) asked for the new
   password after a WRONG current one and failed only at the write.  Now
   `cred-own verify` before the prompts; sudoers `%sdusers` has three lists.
   Witness: M7d2, M10g/h, and the owner's keyboard (wrong current → "Password
   not changed." at once).
3. Instrument: M10b (sudo-rs says "I'm afraid I can't do that"; + M10b3),
...
5. ***[S.31] SD'S PASSWORD RULE — CLOSED 19 SEP 2026: WITNESSED ON
   `60ac74a` (M2e/f, M11a-d) and by the owner at the keyboard (a weak new
   password shown the rule and counted; three end it; a strong one set).*** Owner's ruling here (and so both ports'): SD
   requires a complex password whatever the OS allows, SDSYS and every
   account; his rule: 8+ characters, one each of a-z, A-Z, 0-9 and a symbol
   (other printable ASCII, space included); bytes outside 32-126 fail.  One
   rule, three implementations that must agree: `gpl.bp/pw_complex`
   (`!pw_complex`), `sd-elevate` `pw_complex` (tested through `--dry-run
   pw-check`, 12-row spec table), the port's.  Applied: MODIFY.PASSWORD
   (`set_acc_password`, weak = an attempt, before the repeat), CREATE.ACCOUNT's
   Linux password (`set_passwd` now prompts itself; `sd-elevate setpw <user>`,
   one stdin line, rule re-applied, `chpasswd`), installer sdsys prompt
   (read -s, the installed helper's pw-check, `chpasswd`).  10920 is new in
   the shared space.  NOT governed: a Linux password changed with `passwd`
   outside SD (the OS's rule), and the install end-text's "sudo passwd sdsys"
   fallback (it prints the rule).  Witness passwords all gained a symbol.
   Objection raised and kept: the `pw-check` door exists only for --dry-run so
   the rule has one bash implementation the installer can reuse; it acts on
   nothing.
6. ***[W.11] THE SECURITY-MODEL EVALUATION — WAITING FOR THE OWNER.***  He
...
7. ***[S.38] CLOSED 20 SEP 2026 — §WITNESSED§ ON A REAL DELETE/INSTALL
   CYCLE AND AT THE OWNER'S OWN LOGIN SCREEN: "verified - sdsys appears on
   login screen."***  `witness-absence --commit`, same cycle: ***99 of
   99***, M7c3/M7c4/M7c5 all PASS (uid 1005 ≥ UID_MIN 1000, a real shell, a
   home) where M7c3 failed on the pre-cycle install.  THE MACHINE WAS FIXED,
   NOT THE MESSAGE.
   ***THE OWNER'S RULING, on being shown the finding below***: *"the account
   needs to be created so that users can get to it through the login screen
   to match windows"*.  So 10190 and 10195 keep their wording — they were
   describing the right route all along — and the account becomes one the
   greeter will actually list.
   - **As built**: `installsdai.sh` drops `--system` from the `useradd`, so
     sdsys is allocated a uid at or above `UID_MIN`.  Nothing else about the
     account changes: not in `sudo` or `wheel`, root only through the
     sd-elevate drop-in, the S.26 gates untouched.  ***THE SHELL AND THE HOME
     WERE ALREADY THERE FOR THIS*** (the 18 Sep block says so in as many
     words) — `--system` survived from before the teardown and silently
     defeated them, so the ruling of 18 Sep was implemented and invisible.
   - ***THE THING THAT HAD TO BE CHECKED BEFORE THE FLAG MOVED, AND WAS***:
     `sd-elevate` refuses sdsys as a target BY NAME (`require_sd_user`), and
     its `uid >= UID_MIN` test is an independent guard for other accounts.
     A guard that had relied on the uid test to cover sdsys would have
     stopped covering it on the next install, with nothing to show for it.
     A note now sits at both rules saying they are independent.
   - **An existing sdsys keeps its uid**, so a keep-accounts cycle over an
     older tree leaves it hidden.  Changing a live uid would orphan every
     file under `/usr/local/sdsys` and `/home/sdsys`, which an installer has
     no business doing unasked — so it prints the console route instead and
     says a delete/install makes it afresh.
   - **Free**: `test-sd-elevate` 138/138, `bash -n` clean on the installer.
     ***AND IT CAUGHT TWO MORE ROWS THAT MEASURED THIS BOX***: `groupadd
     sdssh|sdapi` were ALLOW rows that passed only while no install had made
     the groups and began failing at 14:12 today — the same trap S.29 part 1
     deleted a row for, in the other direction.  They now assert the
     whitelist without asking the machine: the helper may allow or refuse for
     existence, but must never answer *"not an SD group"*.
   - ***WITNESS M7c3/M7c4/M7c5, MEASURED TWICE.***  Before the cycle: `uid=999,
     UID_MIN=1000` → M7c3 FAIL, the defect stated by the instrument rather
     than by a person.  After the delete/install (`b8e195d`, `witness-absence
     --commit`, 20 Sep): `uid=1005, UID_MIN=1000` → 99 of 99, M7c3/M7c4/M7c5
     all PASS.  ***AND THE OWNER CONFIRMED IT AT THE SCREEN ITSELF***, which
     no instrument can do: *"verified - sdsys appears on login screen."*
   - **Unwitnessed by construction, not by omission**: a KEEP-accounts cycle
     over an sdsys made before this fix.  This cycle removed accounts, so it
     could not exercise the "existing sdsys keeps its uid" branch; that is
     the one path M7c3 cannot speak to, and it stays open for whichever
     future cycle keeps accounts over a pre-fix install.
   - *As found:*
   Found by the owner at the machine, 20 Sep 2026, trying to follow the
   instruction in order to reset SDSYS's SD password: *"there is no linux
   sdsys account listed to switch to"*.  Measured here: `getent passwd
   sdsys` → `sdsys:x:999:979::/home/sdsys:/bin/sh`.  ***UID 999 IS UNDER
   `UID_MIN` (1000, `/etc/login.defs`), so it is a SYSTEM account and every
   common greeter hides it.***  10190 and 10195 both say *"Log in as sdsys
   at the machine's keyboard (or a desktop-sharing view of it)"*, which a
   reader takes as the login screen.
   - ***THE ROUTE IS REAL AND UNNAMED: A TEXT CONSOLE.***  Ctrl+Alt+F3, log
     in as `sdsys`, run `sd`.  That satisfies CPROC's test on both halves —
     `pam_loginuid` sets loginuid to sdsys on a real login, and
     `SSH_CONNECTION`/`SSH_TTY` are empty on a VT.  `/home/sdsys` exists
     (`drwxr-x--- sdsys:sdusers`, made 19 Sep) and the shell is `/bin/sh`,
     so nothing about the login is unusual.
   - **Not a capability defect.**  The administrator CAN reach it, so the
     stance's *"a user cannot do X is not by itself a defect — check whether
     the administrator can grant it"* applies.  **It is a wording defect, and
     the worst place to have one: a refusal, read by somebody already
     stuck.**
   - **The port is not the answer here.**  Its wording says "keyboard"
     because on Windows the logon screen really is the way in; Linux hides
     system accounts from the greeter instead.  Same instruction, different
     mechanism — the OS difference, so naming the console is parity, not a
     divergence from it (`parity-means-same-result`).
...
8. ***[S.39] S.38 BROKE A WITNESS ROW THAT ASSUMED SDSYS IS ALWAYS UID 999
   — CLOSED, §WITNESSED§ 20 SEP 2026: FOUND, FIXED AND RE-RUN THE SAME
   SESSION.***  248 of 249 on the re-run, S2.a passing (*"expected 'yes',
   got 'yes'"*); the sole remainder is R3's already-expected NOT REACHED.
   The very next `witness-release-run --commit` after S.38 landed (install
   `b8e195d`) had scored 247 of 249, with one genuine failure: S2.a, *"WHO
   ...
   - ***RE-RUN, SAME INSTALL, NO DELETE/INSTALL NEEDED***: `witness-release-run
     --commit`, `/var/tmp/witness-release-run.20260920-155539.log`, ***248 of
     249*** — S2.a passes and no new failure appeared where it used to be
     reached, so the `setpriv --groups`/`--regid` arguments were not hiding a
...
(needs a keep-configuration cycle).

***THE PREVIOUS HANDOFF, 19 SEP 2026, ~01:30 — HISTORY NOW.***
...
1. `24d6611` - `installsdai.sh:788` chowned `$cred` to `sdsys:sdsys`, a group
   that DOES NOT EXIST; the first cycle died there.  Now `sdsys:sdusers` (mode
   700, so the group confers nothing either way).
2. `2908280` - `createa`'s make.account loop had lost the `end` that closes
   `read rec from newvoc.f, id then`, so the program did not compile, `$CREATEA`
   had no object, and the install died seeding the account.  Restored.
3. `7bd558d` - A4: `createa`'s `set.owner` chowned with the `OS$CHOWN` intrinsic,
   which needs root, and the administrator is a sdsys session and is never root
   (S.26) - so accounts created after an install came out `sdsys:sdusers`
   instead of `<account>:sdu_<account>`.  `sd-elevate` gained a validated
   `chown-account <user> <group> <path>` and `set.owner` routes through it when
...
sudo groupdel sdu_zzrel2                      # DELETE cycle does NOT take the
sudo rm -rf /home/zzabst /home/zzacct2        # sdu_ groups or the homes away
cd /home/don/Projects/SDCoreLinuxProject/sdcore4linux
bash deletesdai.sh    # y . Keep accounts? n, then type DELETE . Enter . n
bash installsdai.sh   # password . y . ssh y/N . API y/N . don's SD password twice . Restart? y
# after the reboot:
cd sdb_ai/sd64/gplbld
sudo bash witness-absence.sh --commit 2>&1 | tee /var/tmp/witness-absence.$(date +%Y%m%d-%H%M%S).log
sudo bash witness-release-run.sh --commit
sudo bash witness-accounts.sh --commit
```

**Traps that have already cost time.**  `installsdai.sh:152` refuses a retry
while `/usr/local/sdsys/bin/sd` exists, so an aborted install needs
`deletesdai.sh` first.  The installer CLONES `origin/main`, so an unpushed fix
is tested by nothing.  `witness-absence.sh` writes no log of its own (hence the
...
**Why this session ended.**  The owner is discontinuing the agent that worked
here after the WINDOWS-side agent had him run a cleanup command that deleted
that project's tree (recovered over about an hour by another agent).  NOTHING IN
THIS REPOSITORY WAS TOUCHED BY IT - verified at handoff: clean tree,
`HEAD == origin/main`, every key path present.  Treat any cleanup recipe from
...
***HANDOFF, 19 SEP 2026, EARLY — THE THIRD CYCLE RAN; ONE DEFECT; FIXED;
THE FOURTH CYCLE IS NEXT.***  Install `8fbe9d8` (00:45:42), `assert-current`
current, `sd.service` active.  ATTACH seeded `don` right: register `don`
→ `/home/sd/user_accounts/don`, group `sdu_don`, dir and voc
`don:sdu_don 2775`, no marker left.  `witness-absence` (owner-run, sudo)
30/50: M2's CREATE.ACCOUNT as sdsys aborted at "Unable to create dictionary
part of $hold" — `createa:861`'s `os.execute` passed
`.../zzabst/$hold` unquoted, the shell ate `$hold`, and `sd-elevate`
legally chowned the ACCOUNT DIRECTORY (disk: dir `zzabst:sdu_zzabst`,
`$hold` still `sdsys:sdusers`).  `7bd558d` added that shell route without
the tree's `!valid_os_path` rule, which would have refused `$` loudly.
Fixed by single-quoting (and refusing an embedded quote).  Also found and
fixed: 10176/10181 printed their line breaks as raw field marks (cproc now
displays line by line, `copyp:262`'s idiom); M8f's anchor spanned a line
break; M3c and release-run A5 passed on a MISSING register record (null case
now refused).  M8f's refusal itself was right (M8g/M8h pass).  Release-run
and accounts were not run — they would cascade on the same M2.  Noticed,
not acted on: `sdu_don` holds `root,don` but no `sdsys`, though createa asks
for all three (same on 14 Sep).  THE FOURTH CYCLE — clear zzabst first
(user, group, dir, home; no register record), then as before.  ***AND
THE INSTALL-TIME SD PASSWORD IS BACK, REQUIRED*** (owner, via the port's
01:40 mail, their RELEASE_1.1 70; held until he lifted it, then built into
the same fourth cycle): `installsdai.sh` asks don's SD password after
sdsys's Linux one, through MODIFY.PASSWORD (never an argument), up to three
times, verified by `$cred/<name>` appearing; a kept credential is left
alone.  Wording: on Linux it is the API's password only - ssh uses the Linux
one.  The bridge it rides was granted on this cycle (M8d); the step itself
is UNMEASURED - no instrument can drive a hidden prompt, so the witness is
the owner at the keyboard plus `sudo test -f /usr/local/sdsys/\$cred/don`.
No twin of the port's 10089 here (no `require.credential`).  Also:
`set_acc_password:145`/`:162` still advised `su - sdsys`; now the login.
...
***SUPERSEDED 19 SEP: THE SD PASSWORD IS BACK AND REQUIRED — the install
asks TWO passwords again (sdsys's Linux one, then don's SD one); see "19 SEP
2026, EARLY".*** ~~AMENDED 18 SEP 2026, LATER: THE INSTALL NOW ASKS ONE PASSWORD, NOT TWO.~~
Owner, adopting the Windows port's ruling of the same night: no SD password
at install — a Linux login already reaches the account, and LOGIN has no
`require.credential`.  `installsdai.sh` keeps the `sudo passwd sdsys`
prompt and drops don's MODIFY.PASSWORD step (and with it the install's only
use of the root loginuid bridge; the witnesses still use theirs).  The end
text says the SD password is the API's only and how an administrator sets
it.  `bash -n` clean; unmeasured, like the rest of the third cycle.  Where
this handoff says "TWO passwords" / "don's SD password", read one.  Also
from the same mail (Windows' ATTACH): our ADOPT already does the job;
reply sent, name/route-keyword convergence pending their answer.
...
decision, 18 Sep 2026). CLOSED 19 SEP 2026 — WITNESSED ON `60ac74a`, the
seventh cycle: absence 72/72, release-run 228/229 (R3 expected), accounts
32/32.*** Built 18 Sep 2026, one change set (see the
...
10919; added: 10176 (root refused), 10177 (sdsys remote refused) *(written
here and NOT wired until 19 Sep 2026 — deleted as an orphan that evening,
S.34, then restored and wired the same night under S.35 when the owner ruled
sdsys must have no remote access; the claim is true now, by a later route than
it assumed)*, 10178/10179/
...
THE WAY IN. CLOSED 19 SEP 2026 — WITNESSED ON `60ac74a` (seventh cycle,
M8a-h: root refused 10176, sdsys login granted, `sudo -u sdsys` refused
10181); the route as the owner corrected it the night it was built.*** Built 18 Sep 2026: cproc's root-entry block refuses a
...
LIMITS ONLY. CLOSED 19 SEP 2026 — WITNESSED ON `60ac74a` (seventh cycle,
M4a-c: the switches refused, SH runs for a plain account).*** Built 18 Sep 2026: op_sh's os_permitted() and its
...
SDSYS; SDSYS LOCAL ONLY. CLOSED 19 SEP 2026 — WITNESSED ON `60ac74a`
(seventh cycle: M5, M7a-h, release-run's API and E rows).*** Built 18 Sep 2026: the ssh boundary is one
...
***[S.30] ADOPT RENAMED ATTACH, AND THE ATTACHED NAME FOLDED — CLOSED 19 SEP
2026: SEEN RUNNING in the seeding of two installs (00:45, and `359d5ce`:
`/home/sd/user_accounts/don` `don:sdu_don` 2775).*** 19 Sep 2026, agreed with the port by mail (its 00:10): the
...
***[W.10] SELF-SERVICE MODIFY.PASSWORD — RULED AND CLOSED 19 SEP 2026:
WITNESSED ON `60ac74a` by the seventh cycle (M7d2, M10a-h) and by the owner
at the keyboard (wrong current refused at once; own password set; another
account's refused 2001).***  The owner:
match the Windows RESULT, not its process; one visible process for the
administrator and the user.  Built the Linux way: `sd-elevate cred-own
query|set` (identity from sudo's SUDO_USER/SUDO_UID; the caller's own
register record only, not suspended, a `sdu_<self>` account; keys on stdin
from a file in a private 0700 dir, never argv; current password proved by its
StoredKey, derived by the caller from the reported salt/iterations and
string-compared by the helper; write via temp + rename, sdsys:sdusers 0600,
read back; `SD_SYS_ROOT` fixture honoured only under `--dry-run`).
`sdcore.sudoers` grants `%sdusers` exactly those two argument lists.
`set_acc_password`: a non-administrator's own change takes `self.svc`, same
prompts; the administrator path is unchanged.  `newvoc/modify.password`
added (UPDATE.ACCOUNTS brings it to existing accounts on an upgrade).  Free:
`test-sd-elevate` 92/92 (20 cred-own rows, each wanting its own reason);
`visudo -cf` OK.  Witness: M7d2 (the sudoers line) and M10a-f (as the plain
throwaway through real sudo: own query; any other verb refused by sudo; set;
owner/mode; wrong current refused and the key kept; right current accepted).
NOT measurable by instrument: the hidden prompts themselves.  *Earlier, as
first filed:*
...
VARIABLES, ONE FOR THE SERVER AND ONE FOR THE CLIENT — CLOSED, §WITNESSED§
20 SEP 2026.***  The fix is a confirmed ancestor of today's install
(`b8e195d`), which ran hundreds of ordinary sessions — none setting either
variable — with no config-path failure; see the table row for the exact
logs.  Found while looking for a cheap way to reach §16 R3 (below).
...
***(3)'s LINUX HALF IS NOW BUILT TOO, 22 Sep 2026*** — the staging
directory, the zip, and the availability notice; see the S.23 row above for
the detail. §OPEN§ — left: the owner's own read of it, and the Windows half,
which is not this tree's to build.
...
3. ***STAGING REPOSITORIES PER VERSION, THEN A RELEASE ZIP OF EACH*** — one for
   Linux, one for Windows. ***THE LINUX HALF IS BUILT, NOT CLOSED — SEE THE
   S.23 ROW, ABOVE: `SDCore-L1.1-0/`, the zip, and the availability notice,
   22 Sep 2026.*** Left: the owner's own read of the notice and README
   content, and no distribution attempted. The Windows half is not this
   tree's to build.

***[S.22] §BUILT AND §WITNESSED§ CLOSED 22 SEP 2026: ALL THREE SETS
READ.*** In a separate repository, `SDCoreLinuxDocs`
(`github.com/dmontaine/SDCoreLinuxDocs`, `ad1ef1c`), forked from
`SDCoreWindowsDocs` post-gate-48 (its tier-model sweep) rather than the
stale pre-gate-48 copy the day started from. **`GettingStarted` (19 of
19), `Administrator` (14 of 14) and `User` (52 of 52, every page bar the
unchanged copyright boilerplate) have each had a dedicated read this
session**, not just a keyword sweep. `tools/docmap.py`/`tclmap.py`/
`confmap.py` all read 0 problems throughout, re-run clean after the
final commit.
...
**The final sweep of `User`'s remaining ~19 language-reference pages
found more of the same kind of error**, none of it OS-specific: a
login-paragraph example still read `TERM WINDOWS` — `term:183-212` has
no such keyword, fixed to `TERM ANSI`, a real shipped terminfo entry.
Both `config gpl` and `config contrib` were documented as reading a VOC
`X`-record; they don't — `gpl.bp/config` shells out to `less` against
plain files `sdsys/licence` and `sdsys/contrib`, and the VOC's only
`X`-record is `$release`. VOC record-type counts (`32`, `32a`) were
recounted directly from `voc_template`/`newvoc` on disk against the
fabricated-looking figures the pages carried: 424 total not 431, `V`
140 not 144 as a literal type character, `F` 15 not 16, `X` 1 not 3,
`newvoc` 399 not 398. `32a` still listed `grant`/`revoke`/`list.grants`
as verbs `NEWVOC` lacks — gone post-teardown, replaced with a real diff
of `voc_template` against `newvoc`. The same page's "VOC/NEWVOC/
ACCOUNTS/MESSAGES/SYSCOM/QFILE still upper case, next" was stale — all
confirmed lower case on disk already. Pages `19` and `32` gave two
different, both-wrong verb-dispatch counts (97/45 vs 99/42) for the same
underlying fact; a direct count gives 95 `CA` / 43 `IN`, now used in
both places.
...
* MESSAGES — 42 files removed (see the S.25 entry), 5 written (10176, 10177,
  ...
  ***10177 WAS WRITTEN HERE AND NOT CALLED FOR A DAY*** — found 19 Sep 2026 by
  the orphan scan, deleted (S.34), then restored and wired the same night
  (S.35) when the owner ruled sdsys must have no remote access.  It says "SD
  administration needs a local session", which is now CPROC's refusal at the
  door; 10181 keeps the different case it was written for that night, a sdsys
  session without a sdsys login.
...
***THE 40 ROWS WERE RE-POINTED AND THE SESSION CLOSED FOR HANDOFF (`a076571`).***
Done in one commit, each change carrying its reason in the file it changes.
`witness-absence.sh` gained `run_sd_as` (the session's USER is a parameter now,
and M8a/M8c run as root for real - "run_sd root" had been only a label, so the
"root session" ran as sdsys and measured nothing); M3a/M4a/M5a (11 rows) assert
the rewrite's own refusal text (`MODIFY.ACCOUNT Action Must Be Add, Delete,
Suspended or Unsuspend`) instead of the old parse's "Unexpected token"; M6d
accepts "already a member of group sdu_<acc>" (CREATE.ACCOUNT joins the account
to its own group, so the first add has already happened) plus an absent
"Unable"; M7e reads the sudoers RULE lines and not its comments; and its cleanup
asks for `REMOVE.HOME` so a run no longer leaves `/home/<acc>` behind - found by
the first run leaving `/home/zzabst`.  `witness-release-run.sh`: §2b's L2/L3b
assert that `LOGTO sdsys` is REFUSED (S.26) and that the refusal names the one
route in; §13g drives the stores from a sdsys session that has LOGTOed INTO the
ordinary account (the old second leg of that route is gone); §13h (11 rows)
drives `REMOTE.API`/`REMOTE.SSH` as SDSYS rather than root, which had measured
10176's refusal every time; E6 takes the throwaway `$cred/sdsys` file away (no
verb can unset a credential - an empty entry means "leave unchanged",
`set_acc_password:192`) instead of asking for a `Password REMOVED` message the
product has never had; §15 kills the account's sessions and waits before
deleting, so `userdel` is not refused for a uid in use.  `witness-accounts.sh`:
D11 asserts the home SURVIVES a plain delete - `REMOVE.HOME` is the other path,
the owner's 10 Sep ruling - where the row had asked for the other path with this
command.

***STATE AT CLOSE.***  `HEAD == origin/main == a076571`; working tree clean;
`check-stale-leads.py` exit 0; every touched script `bash -n` clean; both light
witnesses' dry runs reach their verdicts and the release run's dry run stops at
its own ground check (the leftover fixtures, not a fault).  The INSTALL is
`2908280`, so `assert-current` reads STALE by two commits and that is the correct
reading: ***A4'S FIX AND THE RE-POINTED ROWS ARE UNMEASURED until the next
cycle.***  The `createa` BASIC edit is likewise unverified - `-internal` needs
root and the unprivileged sandbox needs an account only an install creates - so
the install's own seeding step is its check.

***AND THIS REPOSITORY WAS VERIFIED INTACT WHEN THE SESSION CLOSED***, after the
Windows-side agent had the owner run a cleanup command that deleted that
project's tree: clean working tree, `HEAD == origin/main`, 3315 tracked files,
every key path present, 215 `gpl.bp` sources.  Nothing here was touched; the
