...
| ⬜ | **P.31** | S | `delacc:219`'s cross-reference scan opens every other account's `voc` with no ELSE, so one unopenable `voc` aborts DELETE.ACCOUNT before the confirmation instead of skipping that account — the Windows port's find (its RELEASE_1.1 44), confirmed here by reading; shared BASIC, so upstream too. Cannot fire on this box today (measured: all three register accounts' `voc` open), but a missing account directory fires it even for root, and an account owner can tighten their own directory and abort a delete of any other account. ***NOT FIXED***: the ELSE wants a message number agreed with the port, so it is a parity decision | — |
