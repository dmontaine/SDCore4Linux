...
iteration count. ***WHAT IS NOT PROVEN IS THE FINAL PROOF, THE ACCOUNT ENTRY AND
WHO***: that needs the real password, so it is one command at the owner's
keyboard, with the password read hidden and never on a command line.
