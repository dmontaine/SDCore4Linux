...
***NEXT FREE ID: 31.*** (Read 30 until 13 Sep 2026, after 30 was taken.) Take it from here and increment it; **do not derive it by
scanning.**
...
| | SEV | What | Where |
|---|---|---|---|
| ~~31~~ | **B** | ***CLOSED — §WITNESSED§ 20 Sep 2026 ON `witness-release-run --commit` (`/var/tmp/witness-release-run.20260920-155539.log`): K8 "10188 named zzrel2, whose voc was missing: found 'Account zzrel2 could not be opened'", K9 "and DELETE.ACCOUNT still finished (register record gone)" — both PASS.*** *(Original entry follows.)* ***FOUND BY THE WINDOWS PORT 15 Sep 2026 (its RELEASE_1.1 44) AND CONFIRMED HERE BY READING THE CODE, NOT BY FIRING IT.*** `delacc:219`'s cross-reference scan opens every OTHER account's `voc` — `openpath xacc.path:@ds:'voc' to xvoc.f then … end`, ***with no ELSE*** — so one account whose `voc` will not open aborts the whole verb, before the confirmation, instead of skipping that account. The scan only feeds a warning (`report.xref`), so an unopenable `voc` should be skipped, not fatal to the deletion. Same construct and the same 2007 comment in both ports, so upstream `sdb64` almost certainly carries it too. ***IT CANNOT FIRE ON THIS BOX TODAY, MEASURED 15 Sep 2026***: all three register accounts (`don`, `sdsys`, `zzinterop`) have a `voc` directory that exists and that `don` can open. ***ONE ROUTE FIRES IT HERE, NOT TWO — THE SECOND WAS MY ERROR, CAUGHT BY THE PORT AND THEN MEASURED.*** I had claimed an account owner could tighten their own directory (`drwxrwsr-x <user> sdu_<name>`, PROJECT_STATUS:2573) and so abort an administrator's delete of any other account. ***IT DOES NOT HOLD, BECAUSE THE DELETER HERE IS ALWAYS A ROOT SESSION.*** Measured 15 Sep 2026 as `don`, ADMINISTRATOR tier and in `sdadmin` but not root: `DELETE.ACCOUNT` is refused 2001 at `:101` — the privilege check, ahead of the register read, against a target that genuinely does not exist — and `LOGTO sdsys` is refused 10002. No non-root session reaches the verb, and a mode cannot hide anything from root. ***WHAT REMAINS IS A `voc` THAT IS GENUINELY MISSING***: a register record whose directory or `voc` is gone — a partial delete, or a register restored without its directories — which aborts it for root too, since root cannot open what is not there. *(An `ls …/voc \| grep` of mine appeared to show the verb absent from a personal VOC. That was a ***NULL MEASUREMENT*** — the VOC is a hashed file with two directory entries, not one per record — and it is written down so nobody repeats it.)* ***FIXED 15 Sep 2026***: the ELSE names the account and carries on, `display sysmsg(10188, xacc)`. 10188 and its wording are the port's, taken verbatim because it built first and the port that builds first carries the number (its message of 17:40; no collision here, measured). BASIC, so it compiles at the next install: ~~§OPEN§ until an install compiles it and something fires it~~ — ***witnessed above*** | `sdsys/gpl.bp/delacc:219`; the port's `delacc:199` | | `sdsys/gpl.bp/delacc:219`; the port's `delacc:199` |
| ~~30~~ | **C** | ***CLOSED — WITNESSED 13 Sep 2026 18:16 ON INSTALL `80b4e83` (full cycle, `assert-current` 0): `stat` root:root 644, `verify-accounts` R8 PASS.*** THE SDSYS REGISTER RECORD IS INSTALLED 755, NOT THE 654 THE INSTALLER SETS — FOUND 13 Sep 2026; BUILT SAME DAY: the chown/chmod moved to after the keep-cycle register restore (which ALSO undid it — a second cause the entry below did not name), mode ***644*** not 654 (a data record; `accounts/DON` is 644; 654 dates from the initial import, upstream's "tighten up sdsys file permissions", and gives group execute on data, which grants nothing), and the installer prints the mode it set. Witness: `verify-accounts` R8 root:root 644. Original finding: `installsdai.sh:618-619` does `chown root:root` + `chmod 654` on `accounts/SDSYS`, then `:624` runs `chmod -R 755 "$sdsysdir"` over it. Measured on install `58365cc`: `stat` → `root:root 755 accounts/SDSYS`. Predates §M3 D1 (same order on `ACCOUNTS/SDSYS`). Effect: others gain execute on a data record; owner stays root. Not yet decided whether 654 is even the intended mode (group r-x on a record is odd) — check the port's register ACL before fixing; the fix is ordering (set it after the recursive chmod), falsified if `stat` still shows 755. | `installsdai.sh:618-624` |
| ~~29~~ | **B** | ***CLOSED 14 Sep 2026 — the installer's "two reboots / kickstart" note (was `installsdai.sh:1112-1117`) is removed, with a changelog entry; the fix is WITNESSED ON 12 BOOT STARTS, 13-14 Sep 2026, and on boot 0 of 14 Sep 09:30 re-read the same day (`is-active` → `active`, `sd -start` then `Finished`, no `sd -stop`)*** (journal, measured 14 Sep: boots -15 to 0, each start running until its boot ended or an uninstall; the pre-fix signature, a shutdown within 1 s, last at boots -17 and -16). *(Original entry follows.)* ***`sd.service` DECLARES SD DEAD A SECOND AFTER STARTING IT, SO SD IS LEFT STOPPED — FOUND 12 Sep 2026 WHEN A VERIFIER COULD NOT RUN. BUILT 13 Sep 2026; INSTALLED `6214b0f`, BOOT 1 OF N HELD (10:49:03 start, 0 `-stop`, `sdlnxd` in the unit cgroup, suite ran without the workaround); BOOT 2 HELD on `1c36762` (11:20:36, same three observations); BOOT 3 HELD on `0cf86ee` (11:35:35, after a FULL delete cycle):*** unit is now `Type=oneshot` + `RemainAfterExit=yes` (`systemd-analyze verify` reports nothing for it). ***Witness after install + reboot, no sudo:*** `systemctl is-active sd.service` → `active`, and `journalctl -b -u sd.service` shows `sd -start` with NO `sd -stop` after it; then the suite runs without a workaround. One boot passing is weak evidence for a race — ***leave it open until several boot starts have held***, and only then drop the installer's "two reboots / kickstart" note (`installsdai.sh:1093-1097`). ***RECURRED 13 Sep on the `58365cc` install — both the installer's start (10:29:20) and the owner's post-reboot boot start (10:30:09) stopped within 1 s; the whole suite refused "SD has not been started" until the owner ran the workaround.*** Journal across 25 boots: boot-time starts also failed on 12 Sep boots −16/−14/−11/−8 (numbered as of 13 Sep 10:30), so it is not tied to any change. Journal, measured: every installer start today (19:33:33, 20:55:23, 21:12:15) logs "has been started" then `sd -stop` "has been shut down" within 1 s; the owner's reboot usually brings SD up (11:56, 19:34, 20:56) but ***the 21:12:44 boot did the same***, systemd logging "Started" before sd's "has been started", `Main PID: 1616 (code=exited)`. Cause, from the source, not yet proven by a change: the unit is `Type=forking` with no `PIDFile`, and `start_sd()` forks twice and each child calls `daemon()` (`sysseg.c` fork `:365`/`:399`, daemon `:383`/`:403`), so systemd guesses a main PID, watches it exit, and runs `ExecStop`. The installer already carries the symptom as folklore — *"In rare cases it requires two reboots… kickstart"* (`installsdai.sh:1082-1086`). Likely fix `Type=oneshot` + `RemainAfterExit=yes` (the unit's own comment says `ExecStop` "seems to crash things"); ***falsified if the stop still follows the start with that unit***. Workaround: `sudo /usr/local/sdsys/bin/sd -start`. | `usr/lib/systemd/system/sd.service`, `gplsrc/sysseg.c:343` |
| ~~28~~ | **A** | ***CLOSED — FIXED AND WITNESSED END TO END 12 Sep 2026 19:35 on install `f14919c` (`assert-current` current, built 19:33), owner-run `witness-accounts.sh --commit`, 33/0.*** The transcript is the evidence, not the verdict line: after `DELETE.ACCOUNT zzacct2`, `sd-elevate: gpasswd -d zzacct2 sdadmin` then `... sdusers`, and the survivor's groups measured `zzacct2` alone — ***zero SD groups left***, which also proves the strip is COMPLETE (the only SD groups are `sdusers`, `sdadmin`, `sdu_*`; the own `sdu_` went by `groupdel`). D12/D13 PASS; the borrowed Linux user and its home survive (D10/D11); `don`'s own `sdadmin`/`sdusers` untouched; register back to `DON SDSYS`, DON `COUNT VOC` 418, nothing left behind. *(Original entry follows.)* ***DELETE.ACCOUNT OF AN ADOPTED ADMINISTRATOR LEAVES ITS LINUX USER IN `sdadmin` AND `sdusers`, AND `sdadmin` IS EFFECTIVE ROOT — MEASURED 12 Sep 2026 18:12, owner-run `witness-accounts.sh --commit`, rows D12/D13 FAIL as predicted.*** An account whose Linux user SD did **not** create (only ADOPT reaches this — install-only) is deleted correctly on the SD side — register, directory and its own `sdu_` group all gone (D7-D9), and the Linux user rightly survives (D10/D11, message 10036). ***BUT `DELACC` NEVER REMOVES THAT USER FROM SD'S SHARED GROUPS.*** Its only privileged calls are `sd-elevate groupdel <own sdu_>` (`DELACC:286`) and `userdel`/`userdel-home` (`:317`/`:319`); there is no `sd-elevate delgroup <user> sdadmin`, though `MODIFYA` `leave.sdadmin` (`:616`) and `GRANTA` (`:307`) both do exactly that on a demote/revoke. Measured after the delete: `zzacct2` still in `sdusers sdadmin`. ***WHY THAT IS SEVERITY A, NOT COSMETIC:*** `sdcore.sudoers` gives `%sdadmin` passwordless `sudo /usr/local/sbin/sd-elevate`, and `sd-elevate` gates only on that group, **not on the caller** — so a leftover member runs `sudo sd-elevate passwd don`, which the helper permits (`don` is uid≥`UID_MIN`, not root/sdsys; `test-sd-elevate.py:104` ALLOWs it), sets `don`'s Linux password, logs in as `don` — who is in Linux `sudo` — and `sudo -i` to real root. So "I deleted that administrator" leaves them a path back to root. ***REACHABLE WITH TWO ADMINISTRATORS, WHICH IS NORMAL:*** the installer's own account is the adopted one and cannot self-delete (6026); a *second* admin (made by `CREATE.ACCOUNT`, so SD-created and stamped) deleting the installer's account hits this — the installer keeps `sdadmin`. For an SD-created account it does not arise: the Linux user is stamped, so `userdel` removes it entirely and its memberships vanish with it. ***THE FIX IS SYMMETRY AND IT IS BOUNDED:*** on the "not created by SD, leaving it in place" branch (`DELACC:340`, message 10036), also remove the user from `sdadmin` then `sdusers` via the `sd-elevate delgroup` calls `MODIFYA`/`GRANTA` already use — SD added those memberships (the ADOPT transcript shows `usermod -aG sdusers` and `-aG sdadmin`), so removing them restores the pre-adopt state and touches no non-SD group. ***BUILT 12 Sep 2026 ON THE OWNER'S RULING — "a true deletion strips everything about the account":*** `DELACC`'s 10036 branch now calls a `strip.membership` subroutine for `sdadmin` then `sdusers` (`sd-elevate delgroup`, guarded by `is_grp_member`, reporting 10021/10022) — the identical mechanism `MODIFYA` `leave.sdadmin` uses. The borrowed Linux user is still left in place; only the memberships SD conferred are removed. ***THE BOUNDARY THE OWNER RESTATED:*** an OS account SD *did* create is still deleted whole by the `sd.made.it` branch (`userdel`, 10028) — unchanged; the strip is ONLY the branch where a user SD did not create survives. ***COMPILE IS SDSYS'S, AT INSTALL — NOT DONE IN AN ACCOUNT*** (owner: *"all system compilation is done by SDSYS, not DON even though he is an administrator"*). A dev-binary parse in DON's BP gave 0 errors (with `INT$KEYS.H` staged — the tell that it was the wrong context), but the authoritative compile is the two-stage bootstrap at reinstall; a broken DELACC fails the install visibly, which is the red control. ***UNWITNESSED — needs commit → push → reinstall, then re-run `witness-accounts.sh --commit`: D12/D13 must flip to PASS*** (the survivor holds neither group), the rest staying green. ***PORT: NOT FILED, on the owner's ruling*** (12 Sep) — its `DELACC:315` has the same shape but its admin group is `BUILTIN\Administrators`, which predates SD, so the Linux reasoning does not transfer. Everything else in the witness passed 31/0 including the borrowed-user delete branch, which nothing had reached before. | `sdsys/GPL.BP/DELACC:340` (the 10036 branch), `:286`, `:317`; `sdsys/GPL.BP/MODIFYA:616` (`leave.sdadmin`); `sdsys/GPL.BP/GRANTA:307`; `gplbld/sd-elevate` (`passwd`, `delgroup`); `gplbld/sdcore.sudoers`; `gplbld/witness-accounts.sh` D12/D13; entries 14, 18, 23 |
| ~~27~~ | **B** | ***CLOSED 14 Sep 2026, measured on the 13 Sep full cycle*** (`80bd15c`, stamped 19:11:39, `assert-current` 0): `/home/sd` already existed — born 19:08:55, during the delete — and `user_accounts`/`group_accounts` were created inside it at 19:11:39, the case the old guard skipped; the install completed. *(Original entry follows.)* ***THE FIRST REAL RUN OF PRE_RELEASE 26'S PATH KILLED THE INSTALL AFTER THE TREE WAS COPIED AND STAMPED — FOUND BY RUNNING THE DOCUMENTED FRESH INSTALL, 10 Sep 2026, NOT BY READING.*** `installsdai.sh:603` was `if [ ! -d "$ACCT_PATH" ]` around both account-directory mkdirs, so `user_accounts` and `group_accounts` were made **only when `/home/sd` was absent**. 26 makes `deletesdai.sh` recreate `/home/sd` to hold the saved `sd.conf` on the DELETE path, so `/home/sd` now **exists but is empty**; the guard skipped both mkdirs, `sudo chown sdsys:sdusers "$ACCT_PATH"/group_accounts` died with *"chown: cannot dereference '/home/sd/group_accounts': No such file or directory"*, and `set -euo pipefail` aborted the install **after** the tree copy and the install stamp (written 01:48:39, commit `dc36771`). The half-install left `/usr/local/sdsys/bin/sd` in place, so `installsdai.sh:125` **refuses to re-run** — recovery is `deletesdai.sh` first. No symlink, no systemd units, `sdadmin` empty, bootstrap never ran; `/home/sd` held only `sd.conf`. **Fixed** by making the mkdirs unconditional and idempotent (`sudo mkdir -p "$ACCT_PATH"/user_accounts "$ACCT_PATH"/group_accounts`, `:622`) and refusing a `/home/sd` that is a file by name (`:616-621`), rather than letting it fail below with *"Not a directory"*. **Witnessed in a sandbox, 5/5, against the block extracted from the shipping file with `sudo` stubbed and `ACCT_PATH` in `/tmp`**: absent, empty and populated `/home/sd` all end with both directories; a file is refused, exit 1; ***the control is the pre-fix block (`dc36771:installsdai.sh:601-606`) on the empty state, which leaves both directories missing with exit 0*** — the state the chown then hit. `bash -n` clean, no BOM, 0 CR. **Unrun in the real script** — the retry runs this line. | `installsdai.sh:603-622`, `:631`, `:125`; `deletesdai.sh:140`; entries 26, 24 |
| ~~26~~ | **B** | ***CLOSED 10 Sep 2026*** — ran for real in that day's delete: it completed and `/home/sd` was a directory holding `sd.conf` (PROJECT_STATUS "Done this session"). Seen again 14 Sep on the 13 Sep full cycle: `/home/sd` is a directory born 19:08:55, during the delete. *(Original entry follows.)* ***THE DELETE PATH COULD LEAVE `/home/sd` AS A FILE AND BREAK THE NEXT INSTALL — FOUND AND FIXED 10 Sep 2026, MEASURED IN A SANDBOX.*** Answering **DELETE** to `deletesdai.sh`'s accounts question removes `/home/sd`; the configuration question defaults to **yes**, and its `sudo mv /etc/sd.conf "$acct_path"` then does **not** fail — with the destination directory gone, `mv` **renames the file to `/home/sd`** (measured: `mv f missing-dir` exits 0 and creates `missing-dir` as a regular file). The delete completes looking successful, and the next `installsdai.sh` dies at `mkdir -p /home/sd/user_accounts` with *"Not a directory"* (measured: the same `set -euo pipefail` shape exits 1). **So the documented fresh-install path — the one PRE_RELEASE 24's witness needs — was broken for anyone who pressed Enter at the configuration prompt.** Fixed with `sudo mkdir -p "$acct_path"` before the configuration `case`, so a kept `sd.conf` lands at `/home/sd/sd.conf`, where the installer restores it from; a no-op when the accounts were kept. `bash -n` clean, no BOM, 0 CR. **Unrun in the real script** — the owner's fresh install runs this line. | `deletesdai.sh:140`, `:143`, `:148`; `installsdai.sh:602-604`, `:653-656`; entries 24, 25 |
| ~~25~~ | **B** | ***CLOSED BY OUTCOME 10 Sep 2026*** — after the 15:11 keep-accounts upgrade `sdadmin` still held `don`, no 10037; the delete transcript was not captured (PROJECT_STATUS "Next task"). Still so 14 Sep, after the 13 Sep 23:06 keep cycle: `sdadmin:x:965:don` in `/etc/group`. *(Original entry follows.)* ***THE UPGRADE PATH DELETED `sdadmin` AND NEVER RE-ADDED ITS MEMBERS — FOUND 10 Sep 2026 WHILE PREPARING THE PRE_RELEASE 24 HAND-OVER, FIXED THE SAME DAY.*** `deletesdai.sh:186` removed the `sdadmin` group **unconditionally**, while the `sdsys` user and `sdusers` group below it have always been removed **only when the accounts are deleted** (`:213-241`). On the routine upgrade that SAVES its accounts, the install recreates `sdadmin` **empty** (`installsdai.sh:480-484`) and nothing re-adds an existing administrator — `CREATEA:402-424` adds on creation only, and `installsdai.sh:765` skips `create-account` when the account directory survives. CPROC's grant wants the tier **and** the group (`CPROC:860`), so every administrator would fall to **10037** with `Admin? No`, and there is no in-SD way back: `CREATEA` and `MODIFYA` both need the flag the missing group just failed to set. Recovery is OS root (`gpasswd -a`), which is why this is a lock-out rather than an inconvenience. **It was latent until 9 Sep because `sdadmin` had no members to lose** — measured empty on both the 16:22 and 17:53 installs — and the promote that evening is what gave it one. **Fixed** by making the removal conditional on `keep_accts = DELETE`, mirroring the rule already stated for `sdsys`/`sdusers`; on the fresh path (DELETE) the group still goes with everything else, and PRE_RELEASE 24's seed re-creates both halves. `bash -n` clean, no BOM, 0 CR. **Unrun** — the next upgrade that saves its accounts is the witness: expect `sdadmin` still holding its administrators and no 10037. | `deletesdai.sh:186-205`; `installsdai.sh:480-484`, `:765`; `GPL.BP/CREATEA:402-424`, `CPROC:860`; entries 14, 24 |
| ~~24~~ | **S** | ***DROPPED 15 Sep 2026 BY THE OWNER'S CHOICE, SO CLOSED: PART (2), THE NON-SUDOER REFUSAL, IS NOT PURSUED, BECAUSE INSTALLATION REQUIRES SUDO BY DESIGN.*** ***PART (1) WITNESSED 10 Sep 2026 — THE OWNER'S FRESH INSTALL (stamped `dc36771`, 01:56:55) AND HIS `sudo sd`: NO 10033, `User : don`, `Account : SDSYS`, `Admin? : Yes`, with `ACCOUNTS/DON` field 5 `ADMINISTRATOR` and `sdadmin` holding `don` read off disk. PART (2) WAS NEVER RUN, AND BY THAT RULING NEED NOT BE. BUILT 10 Sep 2026 ON THE OWNER'S RULING OF THAT DAY.*** Two installer changes, both in `installsdai.sh`. **(1) THE INSTALLING USER IS SEEDED AS AN SD ADMINISTRATOR.** `:767` was `create-account USER "$tuser" no.query`, which makes a STANDARD account with no tier and no `sdadmin` membership, so a fresh install had **no registered administrator at all** and CPROC's bootstrap arm (msg 10033) was the only way in — measured 10 Sep: `ACCOUNTS/DON` field 5 `STANDARD`, `sdadmin` empty. The command now carries `ADMINISTRATOR`, CREATEA's own keyword path (`CREATEA:402-424`, PRE_RELEASE 18), which writes `ACC$TIER` and adds the person to `sdadmin` — so the installer duplicates neither half and the arm becomes a fallback. **The port does the same thing through ADOPT**, which its CREATEA also forces to ADMINISTRATOR (`adopt-account.ps1:293`), so this is conformity rather than invention. **The helper call is not new either**: `CREATEA:414` and `MODIFYA:303` make the identical `sd-elevate addgroup <user> sdadmin` call, and MODIFYA's was witnessed on 9 Sep (the promote printed *"don added to sdadmin"* and `sdadmin` gained `don`), so the unrun part is the fresh-install session, not the add. **The existing-account guard is untouched**: an upgrade that saves its accounts keeps the tier it has (`:765`'s directory test skips the create) and nobody is promoted by reinstalling. **(2) A NON-SUDOER IS REFUSED AT THE FIRST SUDO, IN WORDS.** The bare `sudo -v` at `:236` becomes a checked call; a caller who cannot sudo now reads *"This installer needs sudo, and X cannot use it"* and exits 1 **before the first command that changes anything**, instead of sudo's own error and a `set -e` abort part-way through. `bash -n` clean, no BOM, 0 CR. **The witness is a fresh install with accounts deleted** — otherwise `:765` skips the create and (1) is not exercised. Expect `ACCOUNTS/<USER>` field 5 `ADMINISTRATOR`, `sdadmin` holding the person, and **no 10033 on the first `sudo sd` after the install**: the arm's silence is the measurement, because it is the only voice it has. *(During the install itself 10033 is expected and correct — it fires on the `create-account` session, before the tier has been written.)* | `installsdai.sh:236`, `:765`, `:783`; `sdsys/GPL.BP/CREATEA:402-424`; entry 18; port `adopt-account.ps1:293` |
| ~~23~~ | **B** | ***CLOSED 10 Sep 2026 on install `242ae63`*** (PROJECT_STATUS "PRE_RELEASE 23 … CLOSED"). Not recorded as done: the 10054 re-witness on a PROGRAMMER account, owed now that §L1 has taken `BASIC` from STANDARD. *(Original entry follows.)* ***THE OS-ACCESS TIER GATE. BOTH COMMITS BUILT 9 Sep 2026; COMMIT 1 INSTALLED (reinstalled from `origin/main`, `sd` up + `LISTF` ok, `assert-current` flipped to current), COMMIT 2 COMPILED AND — for BOTH gates — NOW WITNESSED BITING (10 Sep 2026, install `242ae63`, STANDARD `pete`): `SH ls` refused **10053** → `MODIFY.ACCOUNT pete SH-ON` (10041) + re-entry runs it → `SH-OFF` (10042) + re-entry refuses again → `MODIFY.ACCOUNT don SH-ON` refused **10039** (admin tier). And the `OS.EXECUTE` gate end to end: `pete` compiled `BP/ostest` = `OS.EXECUTE 'ls'`; `run bp ostest` refused `000000A9: … not permitted to use OS.EXECUTE` (**10054**) → `MODIFY.ACCOUNT pete OS-ON` (10041) + re-entry **ran `ls`** → `OS-OFF` (10042) + re-entry refused **10054** again. (Admin-tier guard not re-run for `OS-ON`; the identical `SH-ON` on `don` gave 10039, same `os.set` check.) ***ENTRY 23 IS CLOSED — both gates refuse → grant → allow → revoke → refuse.*** ***THE OBJECTION BELOW IS NOW RESOLVED BY MEASUREMENT***: every shipped `OS.EXECUTE` caller — editors included — is `$internal` and bypasses via `HDR_INTERNAL` (`op_sh.c:135`), so no shipped verb trips 10054 for a non-admin; the gate bites only a NON-internal (user-compiled) program. ***AND THAT COMPILE IS ITSELF PRE-§L1***: `pete` could build `ostest` only because per-tier VOC is unbuilt here — the port omits the compiler/cataloguer/editors from STANDARD via `NEWVOC`'s `TIER.OMIT.STANDARD` (port `CREATEA`, owner 17 Aug 2026: STANDARD *"can run an application but not build one"*; PROGRAMMER = full VOC; ADMINISTRATOR = full + `TIER.ADD.ADMINISTRATOR`). Re-witness 10054 with a PROGRAMMER account after §L1 — the gate follows the PERSON, the compiler follows the ACCOUNT.*** Owner's model, 9 Sep 2026, confirmed as the port's (its 157/80): *"os.execute and shell are off for anyone but administrators; administrators have full access to ssh, api, os.execute and shell"*, and an admin may grant a named non-admin account with `MODIFY.ACCOUNT OS-ON \| OS-OFF \| SH-ON \| SH-OFF`. ***THE HOLE COMMIT 1 CLOSES: `op_sh()` HAD NO PERMISSION CHECK AT ALL***, so a non-internal user program's `OS.EXECUTE` ran unconditionally — the tier's in-SD boundary did not exist at the C layer. **Commit 1** adds `os_permitted()` to `op_sh.c` = `HDR_INTERNAL \|\| USR_ADMIN`, else refused with new message **10054** (`%s is not permitted to use OS.EXECUTE` — the port's number and wording, `%s` because it is C-referenced through `k_error`). **Every shipped `OS.EXECUTE` caller is `$internal`** (all 11, checked 9 Sep 26), so nothing shipped breaks; the `SH`/`!` prompt path is already admin-only at `CPROC:3490` (`os.command`) and reaches `op_sh` as an internal caller, so it is unaffected and not double-gated. **Built clean, 0 warnings, `bin/sd` relinked; NOT witnessed — the install is STALE and builds `origin/main`, so it needs a reinstall to see the refusal.** ***COMMIT 2 BUILT AND COMPILED 9 Sep 2026, UNRUN.*** `ACC$SH`=7 / `ACC$OS.EXEC`=8 in `SYSCOM/KEYS.H`; `K$SH`=58 / `K$OS.EXEC`=59 (BASIC `INT$KEYS.H`) and `K_SH`/`K_OS_EXEC` (C `keys.h`), with `USR_SH`=0x0100 / `USR_OS_EXEC`=0x0200 (`sysseg.h`) set only from `$internal` in `op_kernel.c`, mirroring `K_ADMINISTRATOR`'s `HDR_INTERNAL` gate. `LOGIN` and `CPROC` `int.logto` load the two flags from the entered account's fields; `op_sh` `os_permitted` adds `|| USR_OS_EXEC`; the `SH` gate at `CPROC:3490` becomes admin-or-`kernel(K$SH,-1)` refusing with **10053** (`%1 is not permitted to use the operating system shell`, the port's wording). `MODIFYA` gains `SH-ON \| SH-OFF \| OS-ON \| OS-OFF` (subroutine `os.set`), refusing an ADMINISTRATOR-tier account with **10039** and reporting no-op/grant/revoke with **10040/10041/10042**. ***COMPILED, WITH A RED CONTROL:*** MODIFYA, CPROC and LOGIN each **0 errors** via `sd -internal BASIC BP <prog>` on a dev binary (the account restored to `COUNT VOC` 410, `BP.OUT` litter cleared); an unbalanced-bracket copy gave **1 error**, so the compiler discriminates. **NOT witnessed on a running system** — needs a reinstall of `origin/main`; the grant also takes effect only at the account's next entry, so the witness is: `MODIFY.ACCOUNT <non-admin> OS-ON` then that account re-entered runs `OS.EXECUTE`; `OS-OFF` refuses again; `MODIFY.ACCOUNT <admin> OS-ON` → 10039. Gate-first ordering (commit 1) meant no `OS-OFF` that claims to deny but does not was ever shipped — the trap `MODIFYA` names where it left `SUSPENDED` out. ***OBJECTION, RECORDED NOT RESOLVED:*** the port says `OS-ON` governs *"OS.EXECUTE and the screen editors"*, but here the editors are `$internal` and so bypass this gate — an `OS-OFF` would not restrain `ED`/`MICRO` on this tree unless commit 2 gates the editor launch separately. Whether that matters is undecided. | `gplsrc/op_sh.c` (`os_permitted`, `sh`); `sdsys/MESSAGES/10054`; `sdsys/GPL.BP/CPROC:3490`; entries 13, 18, 22; port 157/80 |
...
| ~~1~~ | **B** | ***CLOSED 14 Sep 2026 — THE LAST THREE WALKED, EACH MEASURED.*** `upgrade-dicts`: NO COUNTERPART NEEDED — the port's upgrade preserved its data tree, but a keep cycle here rebuilds `/usr/local/sdsys` and `installsdai.sh:895,903` runs `write_install_dicts` + THIRD.COMPILE on every install, and all six FILES_DICTS targets are sdsys files. ***THE WALK FOUND S.11***: `$HOLD.DIC^@ID` never applied since §M3. `clean-deadvoc`: NO COUNTERPART — it removed debris of the port's own `verify-catgate`; this tree's catgate rows create nothing (`verify-sysperms` K4). `restart-sd`: NO COUNTERPART NOW — `systemctl show sd.service`: `KillMode=control-group`, `sdlnxd` in the unit cgroup, so a stop waits for and kills a surviving daemon, the case the port's script existed for; and no `gpl.bp` program restarts SD (the port's caller was REMOTE.API, task table W.4). *(Earlier:)* Partly closed 14 Sep 2026. The testing half is CLOSED into PORT_ADOPTION Queue 22 (all 79 instruments classified; its remainder is task table Q.22). Of the admin half, `upgrade-voc` is Queue 16, the `secure-*` intent is `verify-sysperms` with Queues 13 and 25, `check-install`/`finish-install` are "Not adoptable" and `api-listener` waits on task table W.4. *(Original entry follows.)* ***THE PLAN HAS NO ANSWER FOR THE PORT'S 157 POWERSHELL HELPERS, AND §L IS SCHEDULED WITHOUT THE VERIFIERS THAT PROVED IT THERE.*** Classified 9 Sep from each script's own header: **113 testing, 38 admin, 6 build**. The testing half is 51 `verify-*`, **28 `test-*-units` that test the verifiers themselves**, 18 `probe-*` and 16 harness. Of the 38 admin, 19 are Windows mechanism with no counterpart here, **12 are the `secure-*` ACL family whose INTENT is §L's POSIX security posture**, and 7 have a direct Linux need the plan already schedules (`upgrade-voc`/`-dicts` §F1/F2, `check-install` §F7, `finish-install`, `clean-deadvoc`, `api-listener`, `restart-sd`). The plan mentions none of it: `verify-`, "the suite", "harness" and "verifier" return **two incidental hits in ~1,600 lines**. See §1 | plan §H "Windows-only work"; `sd4windows/sdb_ai/sd64/gplbld/*.ps1` |
...
| ~~3~~ | **S** | ***RULED 10 Sep 2026 (owner): `EDIT` STAYS AN ALIAS FOR `ED`, THE LINE EDITOR.*** The port's `edit` was Microsoft Edit, which is not packaged for Linux and is dropped; the full-screen verbs here are `NANO` and `MICRO`, both `CA $EDIT` (the adopted port program). The near-miss below is therefore a ruled difference, not drift. *(Original:)* **`EDIT` MEANS DIFFERENT THINGS IN THE TWO SYSTEMS, WHICH IS A NEAR-MISS NAME WAITING TO BITE.** Here `VOC_TEMPLATE/EDIT` → `$ED`, the **line** editor. In the port `voc_template/edit` → `$EDIT`, the **full-screen** editor. A user or an agent moving between the two gets a different program from the same word | `sdsys/VOC_TEMPLATE/EDIT` vs `sd4windows/.../voc_template/edit` |
| ~~4~~ | **M** | ***SUPERSEDED 10 Sep 2026: `GPL.BP/MICRO` IS DELETED; THE PORT'S `EDIT` PROGRAM IS ADOPTED FOR `NANO` AND `MICRO`***, and its `find.editor` (`command -v` here) is the fix this entry asked for. Compiled 0 errors, unrun — see PROJECT_STATUS "NANO and MICRO". *(Earlier:)* ***BUILT + COMPILED 10 Sep 2026, UNRUN.*** `find_editor` asks `command -v micro` once and refuses in words; the editor's exit code is reported when no working copy comes back. **Found alongside a bigger defect: `MICRO` ran nothing outside `sudo`** (10053) — see PROJECT_STATUS Open, "MICRO + plain-sd administrator OS access". *(Original: hard-coded `micro`, `execute "!"`, no test of the result; the port's UPSTREAM #16 — *"Record is unchanged"* when the editor is absent, blaming the user's data for a missing binary.)* | `sdsys/GPL.BP/MICRO` `find_editor`, `check_permitted` |
| ~~5~~ | **S** | ***FIXED 13 Sep 2026 (`51da55b`, §M)*** — `bbcmp.py:7150-7153` now lowers the include name (read in the source 14 Sep). *(Original entry follows.)* ***`bbcmp.py` UPPER-CASES EVERY `$include` NAME, SO A LOWER-CASE INCLUDE IS UNRESOLVABLE ON ext4.*** Found 9 Sep 2026 trying to compile `CPROC`: `$include define_install.h` fails because the file on disk is lower case. **This is a third name lookup that plan §M1 does not name** — §M1 lists the colon prompt/query language and BASIC `OPEN`, and stops there | `sdb_ai/sd64/gplbld/bbcmp.py:7141` |
| 6 | **B** | ***DONE 14 Sep 2026 FOR ALL BUT TWO HALVES — `A1` (undo), `A3`, `A5` (the grow path) AND `A6` (both sites) EXERCISED BY INDUCED FAILURE IN A SANDBOX, EACH RED ON A MUTANT: `gplbld/sandbox-txnfail.py` 22/22, run twice with identical numbers (no sudo, live IPC keys unchanged). THE RUN FOUND A DEFECT THE A6 FIX CREATED: a failed OPENSEQ … OVERWRITE stranded its record lock past OFF; FIXED in `op_seqio.c`'s error cleanup the same day, L1 shows no lock after it. §OPEN§, and neither is inducible here: `A1`'s lock release (no locks remain after the failure on the fix AND its mutant, so the probe cannot see it) and `A5`'s free-node READ-failure path (needs a transient read error).*** A6: WEOFSEQ on `/dev/null` and OVERWRITE of a mode-444 file both take ON ERROR with 3010 and the file is untouched; mutant "succeeds" with the old content kept. A3: COMMIT deleting from a mode-555 directory file fails ("Delete error in transaction commit"); mutant returns status 0 with the record still there. A1: the two DH writes before it are back (errlog "3 record(s) restored"); mutant leaves NEWK1/NEWK2 — a half-applied commit. A5: 245-byte I-type keys under RLIMIT_FSIZE 33792 — every failed index-node allocation reported (8506), the index missing exactly those and still exact after 40 more writes; mutant reports 204 failures while 239 are missing, loses 16 of 40 later writes, and hangs on 41..600. *(Earlier:)* ***PARTLY EXERCISED 12 Sep 2026 — `A2` AND `A4` ARE BOTH COMPLETE, by `gplbld/verify-txn.py` (33/33, install `0095937`).*** `A2` write and `A2` delete measured with two instruments (SD reads `,` and `=` back; on disk `%C` and `%E` exist and `%Y` is gone, with no raw-id file). `A4` all three parts: both `R2` and `R3` land; `SYSTEM(1008)` balanced across the pair; and ***`SYSTEM(1007)` names the PARENT again after the inner commit*** — measured 29 → 30 → **29** → 0 in one run (the numbers are allocated per transaction and differ every run, so the rows assert RELATIONSHIPS, never literals). ***THE ROW THAT MAKES IT MEAN ANYTHING IS T10, "the inner has a DIFFERENT number":*** had the inner reused the outer's, "the number came back" would be true however badly `end_txn_level()` behaved, because it would never have changed. ***RED CONTROL: `--probe` with `N3` reporting the LEVEL instead of the number → T11 alone fails (expected 29, got 1), T9/T10/T12 still pass***, so the row is load-bearing and isolated. ***STILL UNEXERCISED: `A1`, `A3`, `A5`, `A6`***, and every one needs an *induced* failure (a read-only record file, a held lock, a damaged index, a failing truncate) — which is sandbox work, not DON work, after the 11 Sep wedge. *(Earlier, and no longer true: "Step 2 (`A1`–`A6`) is committed, compiled and NOT ONE ITEM EXERCISED.")* `A5`'s failure mode is a permanently damaged index and `A1`'s a half-applied commit, **both silent** | PROJECT_STATUS "Step 2" ×3 sections |
| ~~7~~ | **B** | ***DONE 14 Sep 2026 — §M installed on `83e5ccf`.*** `verify-nocase.py` run 14 Sep, no flags: 0 name remnants, 0 of 3 code sites, "§M NAME HALF: COMPLETE". *(Original entry follows.)* ***§M, THE LOWER-CASE CONVERSION, IS RELEASE-BLOCKING BY THE OWNER'S RULING OF 9 Sep 2026*** — *"as long as it is done by the end"*. Recorded here as well as in PROJECT_STATUS because the failure mode is silence: deferred once more each step until the port ships with `GPL.BP` and `SYSCOM` still upper case. ***WIDENED 11 Sep 2026 (owner): COMPLETE, NOT THE PORT'S RESULT*** — everything lower case, including files `CREATE.FILE` makes and program/include names, input converted on the fly, so no name can exist in two casings; the port's partial conversion is filed to it as a bug. Measured gaps and the two unruled questions (users' own record ids, account names) in PORT_ADOPTION "Queue 18" | plan §M; PORT_ADOPTION queue 18; PROJECT_STATUS "Open" |
...
| ~~9~~ | **S** | ***DONE 12 Sep 2026 (`3d69e7f`)*** — rewritten for this tree's shapes as PORT_ADOPTION queue 21. Run 14 Sep with no arguments: 457 entries scanned, exit 1 on its single lead, PORT_ADOPTION row 19, already read and accepted. *(Original entry follows.)* ***`gplbld/check-stale-leads.py` CANNOT RUN HERE AT ALL, AND ADDING THIS FILE DOES NOT CHANGE THAT*** — measured 9 Sep 2026, not predicted. Copied verbatim and run, it exits **2 before any phase executes**: *"REFUSING - could not bound section 7"*. It is keyed to the port's PROJECT_STATUS structure — a section 7, `> ###` START HERE items, a `✅` task table — none of which exists here. **The unadapted copy was removed rather than committed**, because a tool that always exits 2 reads as a guard the project has. See §9 | `sd4windows/sdb_ai/sd64/gplbld/check-stale-leads.py` |
| ~~10~~ | **M** | ***NOT APPLICABLE HERE - measured 14 Sep 2026:*** nothing in `sdb_ai/sd64` defines or raises 4100, 4101 or -10303 (grep of the whole tree, build output excluded; control: 3001 found in `gplsrc/err.h`). They are the port's `ER_SRVRERR`, `ER_INV_NBR` and `SD_SCRAM_ERR` (its `gplsrc/err.h:155-156`, `:302`), and the port's `messages` lacks all three too. *(Original entry follows.)* **`sdsys/MESSAGES` lacks records `4100`, `4101`, `-10303`** (plan §D5). That is the runtime message file, not generated from `err.h`, so `gen_includes.py` does not touch it; adding the three is a deliberate data edit | `sdsys/MESSAGES/` |
| ~~11~~ | **M** | ***CLOSED 14 Sep 2026 — the bound is DERIVED from `sddefs.h`, `k_error.c` and `messages.c` on every run, each premise printed with its `file:line`, and a missing premise is a refusal (exit 2).*** Still 231 on this tree. `gplbld/test-msglen-units.py` 10/10 changes each premise in a copy of the source; its first run FAILED case 5 (a changed prefix refused instead of re-deriving, because the prefix text itself was pinned) and the checker was fixed. Now a free check. *(Original entry follows.)* **`gplbld/check-msglen.py` hard-codes the bound 231 and will not say so if the constants move.** All four were verified against this tree when it was ported on 9 Sep, but nothing re-checks them; a change to `MAX_ERROR_LINES`, `MAX_EMSG_LEN`, the `"%08X: "` prefix or the D1 fix leaves a confident instrument answering from a stale premise | `sdb_ai/sd64/gplbld/check-msglen.py` |
...
| ~~16~~ | **S** | ***CLOSED 14 Sep 2026 — the owner's keep cycle installed `6e5b2f5` with it (stamp 12:20:13, `assert-current` current); the installer stops at the first failure (`set -euo pipefail`), so a stamp means `make -B` succeeded as the caller. The installer transcript itself was not pasted.*** *(Built state follows.)* Built 14 Sep 2026: `installsdai.sh` runs `make -B` as the calling user (was `sudo make -B`); only the copies into `$sdsysdir` use sudo.*** Checked: the Makefile writes only inside the tree (no `/usr`, `/etc`, `chown`, `install`); a fresh `git clone --depth 1` of `4d38145` with the installer's `sdext_python_inc.h` built as `don` with `make -B` — exit 0 in 5 s, `bin/sd --version` runs, `find . -not -user don` 0. Both `sudo rm -fr` cleanups kept (the first also clears a root-owned clone from an older run). ***Not checked:*** `sudo python3 gplbld/pcode_bld.py` still writes `pcode_bld.log` root-owned into the clone, harmless since the cleanup is sudo. ***Witness:*** the next install prints "Successful Build." and ends without an error. *(Original entry follows.)* ***THE BUILD RUNS AS ROOT AND DOES NOT NEED TO*** — `installsdai.sh:359` is `sudo make -B`, so `gplobj/` and `terminfo/` inside the download come out owned by `root`. That is what made the 9 Sep install "fail" after it had succeeded: the ordinary-user `rm -fr` at the end could not remove them, returned 1, and `set -euo pipefail` aborted with no message. **Fixed by making the two cleanups `sudo rm -fr`, which treats the symptom.** The cause is that compiling needs no privilege at all — only *installing* does. Building as the calling user and `sudo`-ing just the copy into `/usr/local/sdsys` would remove a whole class of this | `installsdai.sh:359` |
| ~~15~~ | **S** | ***ANSWERED 9 Sep 2026 by entry 8.*** `assert-current` checks that the tree is committed, that HEAD is `origin/main` and that the install stamp is HEAD, so an install built from anything else reads STALE; CLAUDE.md's constraint now says the installer clones `main`. *(Original entry follows.)* ***AN INSTALL NOW TESTS `origin/main`, NOT THE WORKING TREE — SO COMMIT AND PUSH BEFORE TESTING, OR YOU ARE TESTING SOMETHING ELSE.*** Owner's decision, 9 Sep 2026: the installer always clones `main` from GitHub. That **reverses plan §F9**, which removed the download precisely so an install would build the bundled `sdb_ai/` tree, and it reverses CLAUDE.md's *"builds from the `sdb_ai/` tree bundled in this repository, not from a clone."* The decision is the owner's and stands; **the consequence is that uncommitted work is invisible to an install and nothing detects that.** The port's answer to the same class of problem is `assert-current` (entry 8), which refuses to test a tree source has moved past. **Until something checks, the discipline is manual.** CLAUDE.md's project-constraint wording needs correcting to match | `installsdai.sh`; plan §F9; CLAUDE.md "Project constraints" |
...
| ~~13~~ | **B** | ***CLOSED 10 Sep 2026*** — live ssh witness on install `d791b4c` (§13 "WITNESSED LIVE"), re-confirmed on `242ae63`. ***RULED 9 Sep 2026 — A STANDARD ACCOUNT DOES NOT GET A REAL LOGIN SHELL; THE TIER IS TO BE A BOUNDARY*** (owner's selection), ***WHICH COMMITS SD TO WRITING `sshd_config`*** — the port's fenced block + refusing preflight is the model. ***BUILT 10 Sep 26, UNIT-TESTED, SYNTAX-WITNESSED ON THIS BOX'S sshd, AND LIVE-ssh-WITNESSED 10 Sep 2026 — `pete` (STANDARD, `sdusers` not `sdadmin`) forced into `sd` with no shell, `don` (admin) free to a normal shell, over real ssh on 127.0.0.1 (see §13 "WITNESSED LIVE").*** Mechanism and PROGRAMMER's case were ruled 9 Sep (see §13 "Ruled 9 Sep — the mechanism"): `ForceCommand` into `sd` via a fenced `Match Group sdusers,!sdadmin` block; PROGRAMMER = STANDARD over ssh; admins (`sdadmin`) excluded. **New standalone helper `gplbld/ssh-forcecommand.sh`** (`--check`/`--install`/`--remove`), installed root-owned to `/usr/local/sbin/ssh-forcecommand` beside `sd-elevate`. Fenced block appended to `/etc/ssh/sshd_config` (universal path — **NOT the `sshd_config.d` drop-in, which is not universal across the four distro families the installer is regaining**, owner 10 Sep); candidate validated with `sshd -t -f` **before** the live file changes (never corrupts it); refuses when no `sshd` found (no verdict from an unrun check) or when the binary `ForceCommand` points at is missing; backs up to `sshd_config.before-sd`; reloads `ssh`/`sshd`. **Refusing preflight**: refuses a pre-existing `ForceCommand` or a `Match` naming `sdusers`/`sdadmin`; deliberately tolerant of unrelated `AllowGroups`/`DenyGroups` (that predicate is the judgment call inside the ruling — stated in the script header, widen/narrow deliberately). Installer wires `--install` near the end, **non-fatal** (a customised-config refusal warns, does not abort a complete install); uninstaller runs `--remove` before deleting the helper, always. `test-ssh-forcecommand.py` **16/0** (5 refusals, 5 writes, 6 state checks), incl. byte-stable round-trip and rejected-candidate-never-reaches-live. ***WITNESSED:*** `sshd -t` exit 0 on our exact block, OpenSSH **10.3p1**; `sshd -T -C user=` activates a `Match User` block (matching→forcecommand, non-matching→none) — `Match`-eval works. ***NOT witnessable offline:*** the group binding — `sshd -T -C` rejects `groups=` on 10.3 ("Invalid test mode specification"), so the non-admin-forced / admin-free behaviour needs a LIVE ssh login by real STANDARD + administrator accounts with sshd running (owner's witness, like the other tier gates). Inert until committed+pushed+installed (`origin/main`, PRE_RELEASE 15). ***Doc line-refs `installsdai.sh:254-296` were STALE*** — the installer is Debian-only now, one apt branch at `:307`. ***ssh IS AN UNGUARDED WAY PAST THE TIER MODEL, AND THE INSTALLER TURNS IT ON.*** Every distro branch installs an ssh server (`installsdai.sh:254,272,285,295`) and the Arch branch starts and enables `sshd` (`:265-266`). SD users are **ordinary Unix users** — `CREATEA:331` does `usermod -aG sdusers` on an account that already exists, so it keeps its login shell. **Nothing in this project writes `AllowGroups` or `ForceCommand`** (grep: zero hits across the installer and `GPL.BP`). So a STANDARD account that §L1 denies `SH` and `!` **just ssh's in and gets a shell**, never touching SD. This is the exact failure the port measured on 21 Aug 2026 — *"a stock sshd_config: no AllowGroups and, worse, no ForceCommand, so an sdsshonly account got a PowerShell prompt"* — except here it is the **default state rather than a regression**. ***MEASURED 9 Sep 2026: every SD account has a real login shell*** (`don` `/bin/bash`, `sdsys` `/bin/sh`, none `nologin`), so §L1's verb withholding is **a convenience, not a boundary**. `sshd` is inactive on this box, which makes the exposure latent here but not absent. See §13 | `installsdai.sh:254-296`; `sdsys/GPL.BP/CREATEA:331`; plan §L5, §H:854, plan:17 |
...
### WITNESSED LIVE 10 Sep 2026 — the group binding, on real ssh logins

The one thing `sshd -t`/`-T` could not show offline (the `Match Group
sdusers,!sdadmin` binding — 10.3 rejects `groups=` on `-T -C`) is now witnessed
on real logins, owner at the keyboard, against the `d791b4c` install.

Setup — a STANDARD account that is a plain non-admin: `no.query` refuses to
auto-create the OS user (`CREATUSR` off → `CREATEA:186` stops 6074), so the OS
user `pete` was made first with `useradd`, then `create-account user pete
no.query` inside `sd`. `pete` is in `sdusers`, not `sdadmin`; `don` is in both.

- **Positive path (the account that must be forced):** `ssh pete@127.0.0.1` →
  the SD banner and `:` prompt, and `off` closed the connection. **No shell was
  reached** — the ForceCommand fired for a member of `sdusers` not in
  `sdadmin`.
- **Control (the account that must NOT be forced):** `ssh don@127.0.0.1` → a
  normal Ubuntu login shell (`don@gitorli:~$`), because `!sdadmin` excludes him.
  The control matters: it shows the block discriminates rather than forcing
  everyone into `sd`.

OpenSSH 10.3p1, sshd started for the test (`systemctl start ssh`; it is inactive
on this box by default — no branch enables it, `installsdai.sh:302`). This
closes the "NOT witnessable offline" gap named above; the row and START HERE are
updated to WITNESSED.
