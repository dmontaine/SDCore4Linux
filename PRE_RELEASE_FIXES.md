...
| | SEV | What | Where |
|---|---|---|---|
| 31 | **B** | ***FOUND BY THE WINDOWS PORT 15 Sep 2026 (its RELEASE_1.1 44) AND CONFIRMED HERE BY READING THE CODE, NOT BY FIRING IT.*** `delacc:219`'s cross-reference scan opens every OTHER account's `voc` — `openpath xacc.path:@ds:'voc' to xvoc.f then … end`, ***with no ELSE*** — so one account whose `voc` will not open aborts the whole verb, before the confirmation, instead of skipping that account. The scan only feeds a warning (`report.xref`), so an unopenable `voc` should be skipped, not fatal to the deletion. Same construct and the same 2007 comment in both ports, so upstream `sdb64` almost certainly carries it too. ***IT CANNOT FIRE ON THIS BOX TODAY, MEASURED 15 Sep 2026***: all three register accounts (`don`, `sdsys`, `zzinterop`) have a `voc` directory that exists and that `don` can open. ***TWO ROUTES WOULD FIRE IT HERE, AND THE PRIVILEGE MODEL IS WHY THEY DIFFER FROM THE PORT'S*** — DELETE.ACCOUNT runs as the administrator's own identity, since ADMINISTRATOR tier plus `sdadmin` needs no sudo (owner, 15 Sep 2026: admin accounts are unrestricted and see other accounts): (1) a register record whose account directory or `voc` is gone — a partial delete, or a register restored without its directories — which aborts it for a root session too; (2) an account owner tightening their own directory, which they own (`drwxrwsr-x <user> sdu_<name>`, PROJECT_STATUS:2573), hiding it from a non-root administrator and aborting a delete of ***any other*** account. ***NOT FIXED.*** The fix is an ELSE that names the un-scannable account and carries on, and it wants a message number meaning the same thing in both ports (the shared-number rule), so it is a parity decision rather than a local edit | `sdsys/gpl.bp/delacc:219`; the port's `delacc:199` |
