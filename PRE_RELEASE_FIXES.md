...
| 6 | **B** | ***PARTLY EXERCISED 12 Sep 2026 — `A2` IS DONE AND `A4` IS TWO THIRDS DONE, by `gplbld/verify-txn.py` (29/29, install `0095937`).*** `A2` write and `A2` delete both measured with two instruments (SD reads `,` and `=` back; on disk `%C` and `%E` exist and `%Y` is gone, with no raw-id file). `A4`: "both `R2` and `R3` land" and "`SYSTEM(1008)` balanced across the pair" measured; ***the third part, `SYSTEM(1007)` naming the parent after the inner commit, is NOT*** — cheap to add, `SYS$TRANS_NBR` is `SYSCOM/KEYS.H:393`. ***STILL UNEXERCISED: `A1`, `A3`, `A5`, `A6`***, and every one needs an *induced* failure (a read-only record file, a held lock, a damaged index, a failing truncate) — which is sandbox work, not DON work, after the 11 Sep wedge. *(Earlier, and no longer true: "Step 2 (`A1`–`A6`) is committed, compiled and NOT ONE ITEM EXERCISED.")* `A5`'s failure mode is a permanently damaged index and `A1`'s a half-applied commit, **both silent** | PROJECT_STATUS "Step 2" ×3 sections |
