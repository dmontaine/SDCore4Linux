...
| | SEV | What | Where |
|---|---|---|---|
| 30 | **C** | ***THE SDSYS REGISTER RECORD IS INSTALLED 755, NOT THE 654 THE INSTALLER SETS — FOUND 13 Sep 2026; BUILT SAME DAY, NOT INSTALLED:*** the chown/chmod moved to after the keep-cycle register restore (which ALSO undid it — a second cause the entry below did not name), mode ***644*** not 654 (a data record; `accounts/DON` is 644; 654 dates from the initial import, upstream's "tighten up sdsys file permissions", and gives group execute on data, which grants nothing), and the installer prints the mode it set. Witness: `verify-accounts` R8 root:root 644. Original finding: `installsdai.sh:618-619` does `chown root:root` + `chmod 654` on `accounts/SDSYS`, then `:624` runs `chmod -R 755 "$sdsysdir"` over it. Measured on install `58365cc`: `stat` → `root:root 755 accounts/SDSYS`. Predates §M3 D1 (same order on `ACCOUNTS/SDSYS`). Effect: others gain execute on a data record; owner stays root. Not yet decided whether 654 is even the intended mode (group r-x on a record is odd) — check the port's register ACL before fixing; the fix is ordering (set it after the recursive chmod), falsified if `stat` still shows 755. | `installsdai.sh:618-624` |
