...
| 4 | **M** | ***BUILT + COMPILED 10 Sep 2026, UNRUN.*** `find_editor` asks `command -v micro` once and refuses in words; the editor's exit code is reported when no working copy comes back. **Found alongside a bigger defect: `MICRO` ran nothing outside `sudo`** (10053) — see PROJECT_STATUS Open, "MICRO + plain-sd administrator OS access". *(Original: hard-coded `micro`, `execute "!"`, no test of the result; the port's UPSTREAM #16 — *"Record is unchanged"* when the editor is absent, blaming the user's data for a missing binary.)* | `sdsys/GPL.BP/MICRO` `find_editor`, `check_permitted` |
