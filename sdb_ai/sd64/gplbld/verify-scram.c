 ...
 * not run.
 *
 * 14 Sep 26 dm - adopted from SD Core for Windows (W.4 phase 1).  Only the build
 * instructions above differ from the port's, which named an MSYS2 command line.
 *
 ...
 * sd, creating a shared segment or starting a listener.  Every later phase of
 * the SCRAM work rests on these six functions, so they are worth being able to
 * test in two seconds rather than after a full build.
 *
 ...
 *
 * The Windows port's docs/SCRAM_AUTH.md
 */
