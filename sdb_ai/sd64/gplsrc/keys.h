...
/* 11 Sep 26 Windows port - PRE_RELEASE_FIXES 16.  Which sessions hold this
   file open, as printable text.  READ-ONLY, so it belongs in the 1000 block
   with the other queries and not with the 10000 actions.  The port's number,
   so FILEINFO keys mean the same in both systems.                          */
...
#define K_AUDIT              57
/* 14 Sep 26 dm - W.4 SCRAM phase 3.  The Windows port's K_SET_USERNAME and
   K_ASSUME_USER, at the port's numbers: after APISRVR's SCRAM proof succeeds,
   set the session user name and become that user.  Both $internal only; see
...
/* 15 Sep 26 dm - S.19.  OPEN.SOCKET flag: TLS 1.3 client, no certificate
   check - for !sdclient, whose SCRAM login binds to the session.  And the
   SOCKET.INFO key that returns that login's c= value ("" without TLS).
   Mirrored in sdsys/syscom/keys.h. */
...
/* 15 Sep 26 dm - S.19.  The SCRAM c= value this API session must carry:
   base64("p=tls-exporter,," + RFC 9266 binding), or "" when the session is
   not TLS (a local pipe connection).  A new number; the Windows port takes
   the same one when it adopts TLS (RELEASE_1.1 41).  No arguments. */
