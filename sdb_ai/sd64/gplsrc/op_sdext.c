...
/* 14 Sep 26 dm - the Windows port's SCRAM-SHA-256 primitives for the API login
       exchange, its cases as written (W.4 phase 1).  Every BINARY value is base64
       - see gplsrc/sd_scram.c for why that is forced rather than chosen - while
       the password given to SD_PBKDF2 and the message signed by SD_HMACSHA256 are
       text, because neither is binary in the protocol either.  Each case is a
       thin adapter: check the argument count, parse any integers, make one call,
       and hand the result to the shared reply path. */
...
/* 14 Sep 26 dm - the Windows port's shared reply path for the sd_scram
   primitives.  Every one of them answers a malloc'd base64 string or NULL, so
   success and failure look the same at every call site.  Note sdme_err_rsp()
   pushes its own empty result, so only the success path touches e_stack here. */
Private void scram_reply(char* value) {
  if (value == NULL) {
    sdme_err_rsp(SD_SCRAM_ERR);
    return;
