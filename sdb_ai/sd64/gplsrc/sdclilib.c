...
#include "sdclient.h"
#include "sdclilib.h"
/* 14 Sep 26 dm - SCRAM-SHA-256 login, the Windows port's (W.4 SCRAM phase 4).
   The primitives are static functions in a header, so this binary gains no
...
/* ======================================================================
   SCRAM-SHA-256 login.  The Windows port's scram_login (its
   gplsrc/sdclilib/sdclilib.c, docs/SCRAM_AUTH.md phase 4), adopted 14 Sep
   2026 for W.4 SCRAM phase 4.  Only its Windows string calls (strcpy_s,
   SecureZeroMemory) are changed, to snprintf and explicit_bzero.
   ...
   use but cannot produce this.  A missing or wrong v= is a hard failure,
   deliberately - it is the check that gets softened while debugging and never
   hardened again.

   THE 600,000 PBKDF2 ITERATIONS ARE PAID HERE, once per connection.         */

/* Server-supplied sizes are capped rather than trusted. */
...
/* A hostile server could otherwise spend the client's CPU for it, or weaken
   the derivation by asking for almost none.  GPL.BP INT$KEYS.H holds the
   server's SCRAM$ITERATIONS separately and nothing cross-checks the two. */
#define SCRAM_MIN_ITERATIONS   4096UL
#define SCRAM_MAX_ITERATIONS   10000000UL

#define scram_error(msg) snprintf(session[session_idx].sderror, \
                                  sizeof(session[session_idx].sderror), "%s", msg)
...
Private void scram_failed(const char* fallback) {
  ...
  if (session[session_idx].server_error == SV_ON_ERROR && n > 0) {
    ...
    memcpy(session[session_idx].sderror, buff->data.abort.message, (size_t)n);
    session[session_idx].sderror[n] = '\0';
    return;
  }

  scram_error(fallback);
}
...
Private bool scram_login(char* username, char* password) {
  ...
  /* 18 bytes: a multiple of three, so base64 adds no '=' padding, and every
     character it emits is inside RFC 5802's printable set. */
  if (!scram_random(nonce_bytes, sizeof(nonce_bytes)) ||
      !scram_b64_encode(nonce_bytes, sizeof(nonce_bytes), cnonce, sizeof(cnonce))) {
    scram_error("Could not generate a login nonce");
    goto done;
  }
  ...
  /* The body is the SCRAM message and nothing else - the packet already
  ...
  if (buff_bytes <= 0 || (size_t)buff_bytes >= sizeof(server_first)) {
    scram_error("Malformed server-first message");
    goto done;
  }
  memcpy(server_first, buff->data.abort.message, (size_t)buff_bytes);
  server_first[buff_bytes] = '\0';

  /* KEPT VERBATIM: server_first goes into AuthMessage byte for byte, so the
     parse works on a copy. */
  memcpy(scratch, server_first, (size_t)buff_bytes + 1);

  r_attr = strtok_r(scratch, ",", &save);
  s_attr = strtok_r(NULL, ",", &save);
  i_attr = strtok_r(NULL, ",", &save);
  ...
  if (strlen(r_attr) >= sizeof(combined) || strlen(s_attr) >= sizeof(salt_b64)) {
    scram_error("Server-first message is implausibly long");
    goto done;
  }
  snprintf(combined, sizeof(combined), "%s", r_attr);
  snprintf(salt_b64, sizeof(salt_b64), "%s", s_attr);

  /* THE CHECK THAT STOPS A REPLAYED SERVER RESPONSE: our nonce must be a
     proper prefix of the combined one. */
  if (strncmp(combined, cnonce, strlen(cnonce)) != 0 ||
      strlen(combined) <= strlen(cnonce)) {
    scram_error("Server nonce does not extend the one we sent");
    goto done;
  }
  ...
  if (errno != 0 || end == i_attr || *end != '\0' ||
    ...
    scram_error("Server asked for an unacceptable iteration count");
    goto done;
  }

  if (!scram_b64_decode(salt_b64, salt, sizeof(salt), &salt_len) ||
      salt_len == 0) {
    scram_error("Server sent an undecodable salt");
    goto done;
  }
  ...
  if (n < 0 || (size_t)n >= sizeof(auth_message)) {
    scram_error("Authentication message too long");
    goto done;
  }

  if (!scram_client_keys(password, salt, salt_len, iterations,
                         auth_message, &keys)) {
    scram_error("Could not derive login keys");
    goto done;
  }
  ...
  /* THE MUTUAL HALF.  expected was computed before the server answered. */
  n = snprintf(expected, sizeof(expected), "v=%s", keys.server_sig);
  if (n < 0 || (size_t)n >= sizeof(expected))
    goto done;

  if (buff_bytes <= 0 || (size_t)buff_bytes >= sizeof(server_first)) {
    scram_error("Malformed server-final message");
    goto done;
  }
  memcpy(server_first, buff->data.abort.message, (size_t)buff_bytes);
  server_first[buff_bytes] = '\0';
  ...
  if (!scram_equal(server_first, expected)) {
    /* NOT A WARNING.  Whatever is on the other end of this socket is not the
       one holding the credential. */
    scram_error("Server failed to authenticate itself - connection refused");
    goto done;
  }
  ...
  explicit_bzero(auth_message, sizeof(auth_message));
  explicit_bzero(salt, sizeof(salt));
  explicit_bzero(nonce_bytes, sizeof(nonce_bytes));
  return status;
}
...
SDConnect(char* host, int port, char* username, char* password, char* account) {
  ...
  /* 14 Sep 26 dm - THE PORT'S SDConnect, W.4 SCRAM phase 4.  SCRAM never
     sends the password, so the only rule left is the port's: an empty one is
     refused - the server has no credential that derives from nothing.  (S.14's
     16-bit bound belonged to the SrvrLogin packet this no longer builds.)    */
  ...
  /* 14 Sep 26 dm - SCRAM-SHA-256 replaces the cleartext SrvrLogin, as in the
     port.  scram_login() sends neither the user's password nor anything
     derived from it that a capture could replay, and refuses the connection
     unless the server proves itself.  It sets sderror on every failure.
     SrvrLogin is still served until phase 5; this client no longer sends it. */
