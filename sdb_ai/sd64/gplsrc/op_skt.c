 ...
 */
  ...
  sock->tls = NULL; /* 15 Sep 26 dm - S.19: k_alloc does not zero */
 ...
 */
  ...
  /* 15 Sep 26 dm - S.19: SKT$TLS is not a protocol bit.  Taken out before
     the protocol tests below, which read flags == 0 as "stream TCP". */
  ...
  if (process.status == 0) {
    ...
    /* 15 Sep 26 dm - S.19: SKT$TLS - a TLS 1.3 client handshake before the
       socket is handed to BASIC.  No certificate check: the caller is
       !sdclient, whose SCRAM login is bound to this session (sd_tls.c).  A
       failed handshake is a failed open, and the socket is not returned. */
    if ((process.status == 0) && want_tls) {
      char tls_err[256];

      sock->tls = sd_tls_client_start(skt, SD_TLS_HANDSHAKE_MS + 5000,
 ...
 */
  ...
  /* Wait for data to arrive.  15 Sep 26 dm - S.19: bytes OpenSSL already
     holds decrypted are invisible to select(), so a TLS socket with some
     pending does not wait for data it has. */
 ...
 */
  ...
  switch (key) {
    ...
    /* 15 Sep 26 dm - S.19: the SCRAM c= value for a socket opened with
       SKT$TLS - base64("p=tls-exporter,," + this end's binding) - and ""
       for any other socket.  !sdclient's login carries it. */
...
// rev 0.9-3 SIGPIPE Error in Sockets issue #89 / ScarletDME
      /* 15 Sep 26 dm - S.19: through TLS on an SKT$TLS socket, all or none. */
      if (sock->tls != NULL)
...
void close_skt(SOCKVAR* sock) {
  /* 15 Sep 26 dm - S.19: close_notify, then the socket. */
  if (sock->tls != NULL) {
