...
| 28 | ***BUILT + COMPILED 14 Sep 2026, UNWITNESSED: an over-long runfile path now raises `10918 Runfile pathname is longer than %d characters` (`op_jumps.c` `op_run`, `n < 0` from `k_get_c_string`), and 1135 is left for an empty name.*** The limit is not lifted. 10918 is Linux-only (the port's 1135 is unchanged, its messages end at 10175); 10916/10917 were skipped because the record names them for the removed `!voccase`. ***Witness after an install that carries it:*** in an account whose `bp.out/<prog>` path exceeds 128 characters, `RUN BP <prog>` should print 10918 with `128`; falsified if it prints 1135 or `Message 10918 not found`. *(Original entry follows.)* `RUN` of a runfile path over 128 characters fails `1135 Invalid runfile pathname` (`op_jumps.c:811-813`, `MAX_PROGRAM_NAME_LEN`) — measured on a sandbox account whose `BP.OUT/HOLD16` was 135. Low: accounts under `/home/sd/user_accounts` are far shorter, and a catalogued verb is found by path (limit 255) | — | lift the limit, or have 1135 say what the limit is |
