...
| **To build — mechanism exists here, NOTHING measures it** | `setpw`, `txn`, `basicfuncs`, `keys`, `lineendings`, `nonet`, `nocase`, `editors`, `createaccount`, `delaccount`, `acctmsgs`, `catgate`, `cmdaudit`, `batchjob`, `logtoaccess`, `sdsysgate`, `sdsyswrite`, `sysdiracl`, `pcodeacl`, `accountacl`, `tierchange`, `notyet` | The worklist. Ranked below. ***DONE: `setpw`, `txn`, `basicfuncs`, `lineendings`, `nonet`, `editors` (items 1-5), `createaccount`/`delaccount`/`acctmsgs`→`verify-accounts`+`witness-accounts` (6), `catgate`/`sdsysgate`/`sysdiracl`/`pcodeacl`/`accountacl`→`verify-sysperms` (7).*** Left, and each blocked on privilege or the single-account state a full cycle left: `sdsyswrite` (a session IN SDSYS, real uid 0); `tierchange` (a fresh `MODIFY.ACCOUNT` tier move — an admin session, and a second account); `cmdaudit` (reads the 0620 audit trail — sudo); `batchjob`, `logtoaccess`, `notyet`, `keys` (backspace) — unbuilt, lower value |
