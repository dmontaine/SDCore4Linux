...
| 14 | TIERGATE, GRANT/REVOKE/LIST.GRANTS, ADD tier ordering, promotion report 10126–10129 (port 57). ***STARTED 11 Sep, TWO FILES WRITTEN, NOTHING COMPILED OR WIRED:*** `GPL.BP/TIERGATE` (`!tier_allows`, the port's, statuses 0-4, SUSPENDED via `ACC$PRIOR.TIER`, unknown tier refused, SDSYS by name) and `GPL.BP/GRP_MEMBERS` (`!grp_members`, the Linux answer to the port's `os_group LISTMEM` — reads `/etc/group` as `!is_grp_member` does; a PRIMARY-group member is invisible to it, said in its banner, and nobody here has an `sdu_` primary). ***STILL TO DO:*** `GRANTA` (GRANT `<acc>` TO `<user>` / REVOKE FROM / LIST.GRANTS over `sd-elevate addgroup`/`delgroup`, which already accept any `sdu_*`; the caller must be in `sdadmin`); three `VOC_TEMPLATE` records `V｜CA｜$GRANTA` + the three names in `TIER.ADD.ADMINISTRATOR`; the gate in `CPROC` `int.logto` (10126, and an audit `reason=tier`) and in `MODIFYA`'s ADD arm (not DELETE — taking access away is always allowed); `promo.snapshot`/`promo.report` either side of the register write (10128/10129); messages 10041-10050 and 10126-10129, ***all free here at the port's numbers***, with 10043 reworded (Linux group membership also reaches a session only at its next login); changelog; compile; install witness | `TIERGATE`, `GRANTA`, `CPROC`, `MODIFYA` | `os_group` → `sd-elevate addgroup/delgroup` + `!grp_members` |
