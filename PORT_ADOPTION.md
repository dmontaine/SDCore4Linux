...
Reference tree: `/home/don/Projects/SDCoreWindowsProject/sd4windows`. Update this file
...
1. ~~**Push and install.**~~ — done 11 Sep: installed as `3bd4421` and witnessed
   (see "Witnessed on the install, 11 Sep 2026"). *(Original:)* Everything
   after `c8409e2` is local. The builds below are compiled, not run; the next
   delete→install cycle is their witness.
2. [W.2] ~~**`DELETE.FILE` with an active select list (message 2050), what Enter
...
4. ~~**§M scope:**~~ — ruled 12 Sep 2026 (see "Queue 18"): record ids in users'
   own data files are out of scope; account names are lower case. §M is done,
   14 Sep. *(Original:)* whether "no two casings" reaches record ids in a user's own
...
| **To build — mechanism exists here, NOTHING measures it** | `setpw`, `txn`, `basicfuncs`, `keys`, `lineendings`, `nonet`, `nocase`, `editors`, `createaccount`, `delaccount`, `acctmsgs`, `catgate`, `cmdaudit`, `batchjob`, `logtoaccess`, `sdsysgate`, `sdsyswrite`, `sysdiracl`, `pcodeacl`, `accountacl`, `tierchange`, `notyet` | The worklist. Ranked below. ***DONE: `setpw`, `txn`, `basicfuncs`, `lineendings`, `nonet`, `editors` (items 1-5), `createaccount`/`delaccount`/`acctmsgs`→`verify-accounts`+`witness-accounts` (6), `catgate`/`sdsysgate`/`sysdiracl`/`pcodeacl`/`accountacl`→`verify-sysperms` (7), `tierchange`→`witness-tierchange.sh` (owner-run 14 Sep, 15/15, `7199726`).*** Left, and each blocked on privilege or the single-account state a full cycle left: `sdsyswrite` (a session IN SDSYS, real uid 0); `cmdaudit` (reads the 0620 audit trail — sudo); `batchjob`, `logtoaccess`, `notyet`, `keys` (backspace) — unbuilt, lower value |
| ***Premise is Windows-specific — measured NOT to apply here*** | `parsertokens` | The port proves a TCL token is not TRUNCATED at a backslash, a Windows **path-separator** bug (`C:\Temp`→`C:`). ***Measured 12 Sep as `don`, no sudo:*** a forward-slash path comes back WHOLE (`CT VOC a/b/c` → `Record 'a/b/c' not found`), a comma still splits (`a,b` → three tokens), and `\` is a STRING-QUOTE character in the BASIC parser (`GPL.BP/PARSER:78`, `or (c = "\")`, alongside `"` and `'`), so a bare backslash is a quote introducer and an unpaired one is 2055 "Unpaired string quote" — **the upstream TCL behaviour, unguarded here, not truncation**. The Linux path case (`/`) is whole, so the bug the port's verifier exists to catch cannot occur; building it would write a passing check for a Windows concern. ***Lead, not a defect claim:*** a record id that legally contains `\` on ext4 cannot be addressed unquoted through PARSER — the same clause. Worth knowing if 2055 ever surprises someone; the code does what upstream defined |
| **Routed to §M / queue 18** | `fold`, `lcnames` | Their intent IS the lower-case ruling; building them separately would fork it |
| **Depends on the API's state here** (W.4) | `apiadmin`, `apiidentity`, `apiname`, `apiport`, `apiremote`, `tierapi`, `scramlogin`, `localconnect`, `peerlog`, `routes` | ***RULED 15 Sep 2026, once W.4 closed: seven are already measured by witness rows, one does not transfer, two were gaps.*** See "Queue 22 — the ten API verifiers, ruled" below |
...
## Queue 22 — the ten API verifiers, ruled 15 Sep 2026

Held UNRULED pending W.4 since 12 Sep. W.4 closed that morning, so each was read
for its own stated intent — its `.SYNOPSIS`, as the other 79 were — and matched
against what this tree measures. ***SEVEN ARE ALREADY MEASURED, ONE DOES NOT
TRANSFER, TWO WERE GAPS.*** No separate `verify-*.py` is built for any of them,
for the reason the harness section already gives: one instrument, not ten copies.

| Port verifier | Its question | Ruling here |
|---|---|---|
| `apiadmin` | does a remote API session hold privileges an ordinary user does not? | **Measured.** An API session carries no `K$ADMINISTRATOR` (`apisrvr:463`, `:1273`); §13c S2–S2e read it off `/proc` while connected — real uid the user's, no root group 0, only its own groups |
| `apiidentity` | does the session actually run as the user? | **Measured.** The same rows; `S2b` is the uid. The port needs S4U and impersonation for this, where here it is `setuid` plus groups and `/proc` is the witness |
...
| `routes` | create-time keyword, MODIFY.ACCOUNT absolute not additive, an administrator gets both | **Measured.** S.16 §13f F1–F9b, including F6's divergence (SSH is not a route SD sets here); RDPACCOUNT has no counterpart |

***THE ONE THAT WAS A REAL HOLE.*** `!valid_os_name` screens the SCRAM user name
BEFORE `$cred` is read (`apisrvr:1100`) and allows letters, digits, dot,
underscore and hyphen only. Nothing had ever sent it a name it refuses. Measured
live 15 Sep as `don`, no sudo, on install `c773008`: `zzname$` and `zz name` are
refused at request 47 — and `zz.ok-name_1`, legal characters with no credential,
is refused in the SAME WORDS. ***The wire cannot say which check fired, and that
is the design*** (`apisrvr:1245`), so the audit reason is what separates them:
§13c gained S9 (the refusal) and S9b (the trail reads `name rejected by
valid_os_name`), with S7b — a legal-charset name audited `no credential` — as
the control that stops S9b passing on a catch-all. Built 15 Sep, unrun until the
next witness. *(The two paths also differ in `status` on the wire, 0 against
...
## Queue 18 — lower case: the owner's ruling and where the port stopped short (DONE 14 Sep 2026; the counts below are the 12 Sep before-state)
...
[S.9] ***CLOSED 14 Sep 2026 — INSTALLED (`6e5b2f5`, re-run on `d704658`) AND
WITNESSED by `witness-release-run.sh --commit` 13:07 on a throwaway with
`$release` at `L0.9-9`: (a) blank line → no 5027, `WHO` → `5 zzrel1`; (b)
`</dev/null` → prompt shown once, finished; (c) field 2 still `L0.9-9`.***
*(Built state follows.)* Built 14 Sep 2026: `login` maps an empty answer to N
(`if s = '' then s = 'N'`, DELETEF 2050's line) and message 5026 reads `Update
VOC to new release (y/<n>)?`.*** N because every prompt built under 3b is Enter
= N and N changes nothing; the port has no ruling on 5026 (searched). Also the
EOF escape, since EOF reads as ''. Message 2050 still lacks `(y/<n>)` (the
