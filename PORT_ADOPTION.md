...
| 22 | Verifier intent (the port's `verify-*`/`test-*` .ps1) — ***HARNESS BUILT, UNIT-TESTED AND WITNESSED 12 Sep: `verify-vocverbs.py` 34/34 on install `0095937`, `assert-current` 0, row B4 FAIL→PASS.*** The 79 instruments are classified below and the worklist is ranked. Six verifiers are NOT BUILT (`cmdaudit`, `sdsyswrite`, `batchjob`, `logtoaccess`, `notyet`, `keys`) and the ten API ones are UNRULED (task table W.4). See "Queue 22" | `gplbld` | Python, per PRE_RELEASE 1 |
