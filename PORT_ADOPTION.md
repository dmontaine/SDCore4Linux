...
| **Depends on the API's state here** (W.4) | `apiadmin`, `apiidentity`, `apiname`, `apiport`, `apiremote`, `tierapi`, `scramlogin`, `localconnect`, `peerlog`, `routes` | The `$cred`/SCRAM half is already "Not adoptable". The API surface was walked 14 Sep 2026 (PROJECT_STATUS, the W.4 paragraph): `tierapi`'s intent and the suspended door are in `witness-release-run.sh` §13b and §14 X6; the rest are not built as separate verifiers |
