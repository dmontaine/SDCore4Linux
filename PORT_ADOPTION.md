...
| 3b | Every Y/N loop maps Enter to its default — **BUILT 11 Sep** for `CATALOG` ×3 (3033–3035), `DELETEF` 6131, `CPROC` 5040, all Enter = N. `SPVIEW` needs nothing (presets `yn = 'Y'` in a formatted field). `DELETEF` 2050: ruled and built 12 Sep, Enter = N, witnessed on `f446ac1` ("Waiting for the owner" 2). **Left for the owner:** `DELETEF` 6133 (Waiting 3), and 2050 in `DELETE`, `COPY`, `CD`, `CT`, `ED` (Waiting 2). **Also not done:** `LOGIN`'s `$RELEASE` prompt 5026 (`login:516-535`, measured 14 Sep) takes only Y or N, with no Enter default and no end-of-input escape; "Queue 3" below says to fix it with this row | | |
